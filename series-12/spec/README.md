...
WS2 proves `Many destWL` on the concrete four-state rank-labelled lift `destWL` (the labelled lift of the plain `destW`) of `ws4-witness-design.md`, with the separated pair `cW` (a reified relatum CARRYING the reified constituent `bW`) vs `aW` (a base relatum), `reify`/`reifyStep`/rank load-bearing, rank NON-INJECTIVE (`aW`, `aW'` share rank 0, so rank is coarser than identity, not a point-indicator), and the separation reinstated as a GENERAL theorem `ws2_many_general` of which the concrete pair is an instance (discipline 4, `spec/design-review-1.md` Finding 1); and the One (`Subsingleton`, `ws2_import_theorem_static`) on any plain, behaviorally-identified, atomless coalgebra without an import via `ws1_atomless_bisim`.
...
- **Tower-layered (the coupling lives in the SHAPE, not the fields, Finding 2):** the `Compass` fields are deliberately free (the values are exogenous), so the layering is NOT a value-constraint on `raise`; it is carried by `ConstituentOf` and `ConvergesUp` (§2.10), which restrict where raising APPLIES to genuine `reifyStep` constituency edges (`part ∈ s.1`, `whole = reify s`, with the pointwise reification fact). Strip `dest`/`reify` and `ConvergesUp` loses its edges: the tower is load-bearing in the definitions, not decorative. The coupling is through the structure of what informs a relatum (the `prec` chain of edges), without collapsing the compasses into one (the values stay free) and without leaving them independent (the chain is only over tower edges).
...
```lean
/-- **The constituency edge (the tower IN the shape, ambient for WS3/WS4, `spec/design-review-1.md` Finding
    2).** `x` is a constituent of `W` iff `W` is the reified relatum of a pattern `s` that `x` belongs to,
    with the pointwise reification fact part of the edge (consistent with the disclosed deviation, §2.5).
    This is where the tower lives in the definitions: `reify`/`reifyStep` are load-bearing, strip them and
    the edge is gone. -/
...
/-- **Convergence (one-layer coherence, the core relation).** Relative to a constituency edge (a part `x`
    that is one of the reified relata a whole `W` stands in relation to, `ConstituentOf dest reify x W`) the
...
/-- **Convergence up the layers (the iterated form).** Coherence along a chain of CONSTITUENCY EDGES from a
    part up to the top of the relevant tower, the "up its layers" reading. Each step is BOTH a genuine
    reification edge (`ConstituentOf`, so `reify`/`reifyStep` are load-bearing in the definition, Finding 2)
    AND a coherence (`Converges`), so "up its layers" follows the layers and strip removes neither. The base
    case is one edge; the model pair (WS4) discharges it on a genuine reification edge, without the tower
    DECIDING the orientation values. -/
...
theorem ws4_underdetermined :                                               -- convergence is UNDERDETERMINED
    ConstituentOf destW reifyW aW bW                                        -- over a GENUINE reification edge …
...
where `destW, reifyW, aW, bW` are the concrete four-state witness (`ws4-witness-design.md`), `bW = reifyW sW` a genuine reified relatum, `aW ∈ sW.1` a genuine constituent (`ConstituentOf`, so the edge is a genuine `reifyStep` edge, Finding 2), and `NonDegenerate` records that `Or` is non-trivial, `orient` is not a collapsed constant, `raise` is not a collapsed constant, and `Converges` is non-vacuous. WS4 owns the model pair; WS5/WS7 consume it (as the audit's `fork` at `underdet`) for the verdict.

`ws4_underdetermined_pair` names the projection of `ws4_underdetermined` that drops the leading `ConstituentOf` and the two `NonDegenerate` conjuncts, the bare holding-and-failing pair `∃ c₁ c₂, Converges … c₁ aW bW ∧ ¬ Converges … c₂ aW bW`, used by WS5/WS6 where only the underdetermination (not the edge or its non-degeneracy certificate) is consumed. Definitionally weaker, cited never redefined.
...
inductive Series12Verdict
  | shapeDrawn          -- the expected verdict: the coincidence shape-honest (forced-for-all vs
...
/-- **The convergence fork (data-level, `spec/design-review-1.md` Finding 4).** WHICH disjunct of the C7
    trichotomy was discharged, each constructor carrying its proof. Data-level (a `Type`-valued inductive)
    so the verdict can CASE on it into a `Type`-valued result. WS4 discharges the `underdet` constructor via
    the model pair; had it landed convergence-decided, `forcedHolds`/`forcedFails` would carry that proof. -/
...
/-- **The verdict BRANCHES on the certified fork (Finding 4).** Not a returned constant: `forcedHolds`/
    `forcedFails` map to `convergenceDecided`, `underdet` to `shapeDrawn`. If WS4 had decided convergence,
    the verdict would say so; because it did not, the verdict is `shapeDrawn` as a THEOREM, not by fiat. -/
...
`shapeDrawn`: WS1 coincidence shape-honest, WS2 `Many` on the rank witness (reification load-bearing) plus the One without an import, WS3 the compass parametric (no distinguished inhabitant, the exogeneity an existential), WS4 `Converges` defined and the fork discharged at `underdet` by a genuine non-degenerate model pair. `convergenceDecided`: WS4 outcome (a), the structure decides convergence, discharged at `forcedHolds`/`forcedFails` and reported in its direction. `Partial`: any side per-instance or degenerate. `Circular`: a WS7 finding (a discipline breached). The verdict is `verdictOfFork` applied to the audit's fork field, so it BRANCHES on the certified disjunct (Finding 4) and cannot be hand-set; `ws5_verdict = shapeDrawn` is then a THEOREM (the fork is `underdet`), not a definitional constant.
...
The **genuinely new Series 12 Lean**, defined here and expanded in the designs: `Opening` and the coincidence (WS1); `Many`, the four-state `dir := rank` witness carrier (`WCar`, `aW`, `aW'`, `bW`, `cW`, `destW`, `reifyW`, `rankW`, `rankLift`, `destWL`), `ws_witness_rank_noninjective`, and the general separation `ws2_many_general` (WS2, over the `ws4-witness` carrier); `Compass`/`ConstituentOf`/`ws3_compass_exogenous`/`ws3_compass_layered` (WS3); `Converges`/`ConvergesUp`/`ws4_underdetermined`/`ws4_wall_is_structural` (WS4); `ConvergenceFork`/`verdictOfFork`/`ws5_name_neutral`/`ws5_verdict` (WS5); and `Series12Verdict`/the `Audit` (WS7). Nothing is `import`ed across series; each transcribed name is copied into the relevant `series-12/formal/Series12/wsN.lean` and re-namespaced, and the closure gate confirms it.

---

*Design index for Series 12. Read this before any `wsN-design.md`; the shared objects, the transcribed carrier, the OPENING (`Opening`, the shape `¬ Recoverable`), `Many`, the COMPASS TYPE (`Compass`, per-relatum, tower-layered, exogenous), the CONVERGENCE RELATION (`Converges`, coherence-up-the-layers), the model pair, and the verdict, are defined here ONCE and cited, never redefined. The five disciplines (§0) are the spine of the review: the coincidence is shape-identity not object-identity; the compass is typed and never evaluated; convergence is defined and proved underdetermined, never decided by definition; inhabitation is a genuine tower-distinction not a point-tag; and the names, consciousness, choice, God, the compass, stay in prose, never proof terms. The verdict fork is shape-drawn / convergence-decided / partial, the verdict a function, never assumed. Series 12's result is the exactly-drawn edge of the undefinable, and the drawing, not the filling, is what it delivers. No em dashes in final academic paper copy; this working design index is not final copy.*
