...
```lean
/-- **THE COMPASS IS LAYERED (two-sided freedom over a genuine tower edge, Finding 2).** Over a genuine
    constituency edge (`ConstituentOf dest reify aW bW`, a `reifyStep` edge with the pointwise reification
    fact), the coupling CHANNEL exists (the edge is where raising applies) and the coupling's VALUE is free
    in BOTH directions: some compass coheres there, some does not. Coupled WITHOUT being one (some compass
    does not cohere) and WITHOUT being independent of the tower (the statement is only about tower edges).
    Quantifier-honest: the concrete compasses live inside existentials (the WS4 model pair, cited not
    duplicated). -/
theorem ws3_compass_layered (hinf : ℵ₀ ≤ κ) (hedge : ConstituentOf (destW hinf) (reifyW hinf) aW bW) :
    (∃ c : Compass (destW hinf) (reifyW hinf) (ULift.{u} Bool), Converges (destW hinf) (reifyW hinf) c aW bW)
  ∧ (∃ c : Compass (destW hinf) (reifyW hinf) (ULift.{u} Bool), ¬ Converges (destW hinf) (reifyW hinf) c aW bW) :=
  ⟨⟨cHold hinf, by simp [Converges, cHold]⟩, ⟨cFail hinf, by simp [Converges, cFail]⟩⟩
```
The layering as a theorem with actual compass content (Finding 2, the `P ∨ ¬P` version deleted): over a GENUINE tower edge (`ConstituentOf`, `reify` load-bearing, so stripping the tower removes the edge), the coupling value is free in both directions. The two existentials are the WS4 model pair used at WS3 strength (both concrete compasses inside existentials, cited from WS4, not duplicated). This is the honest reading of "coupled without being one and without being independent", and it is WS4's underdetermination in seed form, over tower edges only.

- **Ambient:** `ConstituentOf`, `Converges`, `cHold`/`cFail` (WS4 model pair, cited).
- **Success condition (Shape-drawn):** over a genuine tower edge, some compass coheres and some does not, quantifier-honest, the tower load-bearing in the statement (`ConstituentOf`).
- **Failure mode:** *the layering baked to force coherence (deciding WS4 early), or the tower phantom (Finding 2).* Foreclosed: the value is free in both directions (neither built in), and the edge is a genuine `ConstituentOf` (not an arbitrary pair), so `reify` is load-bearing. **Winner (the layering).**

**Paper triage.** Decidable: the two existentials are `cHold`/`cFail` over the `ConstituentOf` edge (the WS4 model pair); the tower is in `ConstituentOf`, not decorative. **Winner.**
...
| C3 | the compass is layered (two-sided freedom over a genuine edge) | `ConstituentOf`, `Converges`, `cHold`/`cFail` | yes, coheres/not over a tower edge | **win (layering)** |
...
-- Recoverable, plainOf, FiniteAttention, RealFor, ConstituentOf, Converges, transcribed / consumed (README §6, §2.10).
-- The dir:=rank carrier (WCar, aW, aW', bW, cW, destW, reifyW, destWL) + cHold/cFail, spec/ws4-witness-design.md
-- (for the existentials only).
...
**Proof architecture.** D1 defines the type, the SHAPE of the compass (per-relatum orientation + raising), held parametric, with the tower coupling carried by `ConstituentOf`/`ConvergesUp` (§2.9/§2.10) rather than by a value-constraint on the fields (Finding 2). D2 (`∃ c`) proves it exogenous: orientations may differ on plain-bisimilar relata, so `orient` is `¬ Recoverable` (the Series 07 necessity), the compass itself inhabiting WS1's opening, an EXISTENTIAL, never a selection. D3 proves it layered: over a genuine `ConstituentOf` tower edge, the coupling value is free in both directions (some compass coheres, some does not), the tower load-bearing in the statement (strip `reify` and the edge is gone). D4 proves the duality with WS2's attention: attention bounded-with-values (knowable), compass free-orientation (typeable-not-evaluable). **Dependencies:** WS1's opening (the `¬ Recoverable` shape the compass inhabits); the tower (`reifyStep`, `ConstituentOf`, `ws3_order_endogenous`); the witness carrier and the WS4 model pair (for the existential witnesses ONLY); WS2's `FiniteAttention` (for the duality). **The parametricity discipline is the deliverable's spine: `Or` and every `Compass` stay parameters; the only concrete compasses are inside `∃`.**
