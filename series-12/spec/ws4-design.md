...
> **BUILD REALIZATION (program-review-2 PR2-S1 / program-review-3 PR3-R2, supersedes the C4 winner text above).** Program-review-1 showed the unconstrained pair below is a typing tautology (the free `raise` field flips convergence on every structure), so the built wall was rebuilt over the constrained `Faithful` class (`raise = id`), and the model pair was INVERTED: the built `cHoldF`/`cFailF` share the identical (identity) RAISING and differ ONLY in the ORIENTATION the structure does not fix (the exact inverse of the "shared orient, differ in raising" design here). `ws4_wall_is_structural` as built proves `(cHoldF).raise = (cFailF).raise` (`rfl`) with opposite verdicts on the orientation. And the "not-seeing = the diagonal's freeness (`ws2_residue_free`)" attribution is FALSE of the build: no WS4 proof uses `ws2_residue_free`; the not-seeing is SERIES 07's import work, as a proof term, `ws4_dissent_is_import` (every faithful dissent at the edge is `¬ Recoverable` of `orientLift`, resting on the plain-bisimilarity of `aW`, `bW`), the fork two-zone at the genuine edge (`ws4_two_zone`, `ws4_decided_within_sight`). The signatures below (`cHold`/`cFail`, shared-orient `ws4_wall_is_structural`) are the retired Phase-B design; the ledger and `ws4.lean` carry the built theorems.
