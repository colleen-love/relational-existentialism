# 2-01b-mechanization — Work Order: The Transfer Theorem and the Design-Ready Group

**This document describes:** `scratch/formal/Spec201b.lean` (new file; imports `Spec201`)
**Normative sources:** `scratch/spec/2-00.md` and `scratch/spec/2-01.md`. Where this document and the specs disagree, the specs win; report, don't improvise.
**Audience:** Claude Code, executing without further clarification.
**Naming note:** this is `2-01b`, not `2-02`, because every item below is normed by specs 2-00/2-01; the name `2-02-mechanization` is reserved for after spec 2-02 (π, quality-weighting, deployment ratification) is written. A few light definitions ARE introduced here out of necessity; they are quarantined in §8's ratification list and must be flagged as `pending ratification (2-02)` in their doc-comments.

---

## 1. Purpose and scope

This order encapsulates the maximum open ground reachable without new design decisions. The centerpiece is the **transfer theorem**: spec 2-01 §4's literature-verified construction of Ω, machine-checked *modulo the existence of the raw final coalgebra*. With L1 already proved, the covariety argument can be formalized in full: **if the raw universe νF exists, then Ω — the universe of coherent objects — exists and is final among coherent models.** That converts T2's status from [blocked: 2.01 construction] to [transfer proved; blocked only on νF existence], isolating the entire remaining gap in one classical, citable fact (Adámek–Trnková) to be attacked in its own order via `MvQPF`.
...
As in `2-01-mechanization.md` §2, plus:

- **New file** `scratch/formal/Spec201b.lean`, `import Spec201` (adjust to the actual module path registered in `lake/lakefile.toml`; register `Spec201b` as a library root following the established precedent, and record the wiring in the mapping table).
...
Spec 2-01 §4, step (3): "Ω is the greatest coherent subcoalgebra of νF" —
...
/-- Spec 2-01 §4 (image lemma): the image of ANY hom from a coherent model is a
...
/-- Spec 2-01 §4 / Spec 2-00 T2 — THE TRANSFER THEOREM. If the raw universe is
...
```lean
/-- Spec 2-01 §3.7 (pre-registered definition, D15): an attention is DEPLOYABLE
...
/-- Spec 2-00 §3, PR-J(b), in the exact form the interface supports — the
...
```lean
/-- Spec 2-00 §2.5 / T12, static skeleton: a SHARED bearing on `r` sits inside
...
Series 3 (2-00 O9); this lemma is the load-bearing static fact under it. -/
...
/-- Spec 2-00 §2.4 / general P1, barrier one (shared-part bound): whenever `y`
...
```lean
/-- Spec 2-00 §2.4 / P2 with content: one shared relation, two genuinely unequal
...
/-- Spec 2-00 C2 (weak form), the definition — pending ratification (2-02):
...
/-- Spec 2-00 C2, SHARPENED: connectivity is NOT interface-forced — the
...
cannot be obtained by definitional accident; it must be earned at 2-02. This
...
Definitions (all `pending ratification (2-02)` — these are the *unweighted shadows* of π; the graded forms need 2-02's quality-weighting):
...
```
-- Specs 2-00 / 2-01 ↔ scratch/formal/Spec201b.lean
...
-- Pending ratification (2-02): Deployable*, RelatesBeyond, StepR, Connected,
--   ContainedIn, Touches, MutualPartial   (*Deployable is 2-01 §3.7's definition,
--   transcribed; the rest are new here.)
-- Deviations from 2-01b-mechanization.md: <list, or "none">
```
