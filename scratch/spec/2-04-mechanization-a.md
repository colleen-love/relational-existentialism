# 2-04-mechanization-a — Work Order: F_C, the Coherence Triple, Ω_C, and the Anti-Mirror

Target file: `scratch/formal/Spec205.lean` (new Lake root, eleventh). Conventions in force: no `sorry`; doc-comment every declaration with its spec address; axiom audit per theorem reproduced in-build; results appended to 2-05 post-hoc-marked with predictions left frozen; hostile-first ordering is **mandatory and is part of this order's deliverables** — the stages below are ordered by hostility, not convenience, and reordering requires a marked note.

The FP table of Spec 2-04 §7 is the contract. Nothing in this order may consult §8 (the D19 register) as motivation for any proof choice; FP4's hostile framing is the only permitted contact.
...
- Doc-comment on the census block: the One cannot merely turn; cite 2-05 §0.
...
- The **holding lemma** stated and proved at interface level (K1 + K2 composition on mixed ties), doc-commented "to bear is to hold," cite 2-05 §2.
...
- Inhabitation: **ω̂₂** (the self-witnessing loop, seed 3 or seed 2 — whichever closes cleanest) closed into Ω_C; `omegaHat2_mem`. Doc-comment retiring `omegaHat_coherent` with a pointer (O-2-05-3).
...
- If FP5 fails: **stop the order.** A second Mirror is a program-level event (2-03 §5.4 has no budget for it); everything downstream is void pending diagnosis.
...
3. The R-C3 split write-up in doc-comments, referee-hostile: what the pair shows (interface-plus-witness: residue not derivable *from this material*), what it does not show (full-Ω_C status open, O-2-05-2).
...
- `boolWitnessC` per 2-05 §6, discharging in one model: FP7 (compiles; `contextsC_differ`; holding lemma non-collapsing — patterns remain distinct).
...
Re-anchoring (EqDepth/T5, Genesis/T10 over Ω_C) — next order (2-05-mechanization-b). S1 pigeonhole, T4 profiles, T7 — queued behind re-anchoring, per 2-02 §7's path. D19 — reserved; no declaration in this file may reference it.
