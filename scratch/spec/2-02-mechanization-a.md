...
**Out of scope — do not attempt:** the S1 pigeonhole (staged next, after this order's `Internal`/`holdDepth` land and 2-02 §6's conjecture statement is refined against them); νF existence / `MvQPF`; T4's profile machinery; C2 about Ω; anything temporal. If an unblocked-looking lemma appears, note it in the PR; do not prove it.
...
/-- Spec 2-02 §3, T13 hearsay bounds. What arrives from `z` through the shared
...
```lean
/-- Spec 2-02 §4: the internal part of a pattern — relations all of whose
...
/-- Spec 2-02 §6 (R3): the depth of x's hold on r — the internalized part of
...
/-- Spec 2-00 §2.5, the musicianship corner: mutual partiality — each holds a
...
-- Deviations from 2-02-mechanization.md: <list, or "none">
```
...
In `scratch/spec/2-00.md` §5, after this order lands:
...
In `scratch/spec/2-02.md` §7: mark this order's items delivered; note the S1 pigeonhole as the sole remaining Series-2 theorem question this side of the νF order.
