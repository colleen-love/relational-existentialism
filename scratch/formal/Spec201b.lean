/-
Spec 2.01b — the transfer theorem and the design-ready group.

Normative sources: `scratch/spec/2-00.md` and `scratch/spec/2-01.md`. Builds on
...
Centerpiece — the **transfer theorem** (T2t): spec 2-01 §4's three-step
...
Several definitions are introduced ahead of spec 2-02 and are flagged
`pending ratification (2-02)` in their doc-comments (work order §8).
-/
import Spec201
...
/-- Spec 2-01 §4. Coherence at a single relation (elementwise; ambient-absolute):
every endpoint of `r` keeps `r` in its pattern. -/
def CoherentAt (Z : Raw) (r : Z.R) : Prop :=
  ∀ x : Z.O, x ∈ Z.endpoints r → r ∈ Z.pat x

/-- Spec 2-01 §4. A good pair of subsets: elementwise coherent and closed under
...
/-- Spec 2-01 §4, step (3): the coherent part of a raw universe (objects side) —
...
/-- Spec 2-01 §4, T2t-i. The union of all good pairs is itself good: the coherent
...
/-- Spec 2-01 §4, T2t-ii (image lemma): the image of ANY hom from a coherent
...
/-- Spec 2-00 T2 / spec 2-01 §4 — THE TRANSFER THEOREM. If the raw universe is
...
/-- Spec 2-01 §4, T2t-iii (companion): the unique morphism lands entirely in the
...
/-- Spec 2-01 §3.7 (pre-registered definition, D15) — pending ratification (2-02).
...
/-- Spec 2-00 §3, PR-J(b) — the DICHOTOMY, in the exact form the interface
...
Propagation over time is dynamics, Series 3 (2-00 O9). -/
...
one holding only the shared relation `true`. Pending ratification (2-02). -/
...
has both among its endpoints. Pending ratification (2-02). -/
def StepR (Z : Raw) (x y : Z.O) : Prop :=
  ∃ r : Z.R, x ∈ Z.endpoints r ∧ y ∈ Z.endpoints r

/-- Spec 2-00 C2 (weak form) — pending ratification (2-02): a raw universe is
...
/-- Spec 2-00 C2, SHARPENED: connectivity is NOT interface-forced — the coproduct
...
earned at 2-02. This is D15 working as intended — an anti-desired-outcome result
...
The *unweighted shadows* of π (graded forms need 2-02's quality-weighting).
All three predicates are `pending ratification (2-02)`. -/

/-- Qualitative full containment (unweighted shadow of π = 1). Pending ratification (2-02). -/
def ContainedIn (M : Model) (x y : M.O) : Prop := M.pat x ⊆ M.pat y

/-- Qualitative touching (unweighted shadow of π > 0). Pending ratification (2-02). -/
...
/-- Spec 2-00 T14, the heart corner: containment one way but not the other —
...
corners and the graded form await π at 2-02 — see the file foot.) -/
...
--     mechanized (the graded forms await π at 2-02).
-/
