/-
Spec 2.03b — Run the Trials (B1, the Forcing Lemma, and the anti-Mirror spot-check).

Normative source: `scratch/spec/2-03.md` §5 (The Trials, R4) — the protocol, the
...
This file is an experiment's lab notebook. The predictions in 2-03 §5.3 were frozen
...
/-- Finite nonempty subsets, as the standing subtype (2-03 §2 shorthand). -/
...
/-! ## FL — the Forcing Lemma (class-level; 2-03 §5.3, §5.4)
...
cannot be artifacts of provisional definitions (2-03 §5.4): any variant evading the
...
arbitrary index family (here `↥S`, the pattern's members). Moral (2-03 §5.3): derived
...
`B1_B` derives via `attBToDiv`. This generality is the acceptance test of 2-03 §5.3/§5.4
...
not reproved. Every gate it passes has no discriminating power (2-03 §5.1). -/

/-! ### B1-A — pure (A), division over the pattern -/

/-- Candidate (A), evaluated at the one-point pair (2-03 §5.1(A)): the relation sort is
...
/-- B1-A — CONFIRMS 2-03 §5.3's Forcing Conjecture for (A): **fails by collapse.**
...
/-- Candidate (B), T possibly empty (2-03 §5.1(B)): the seen `p.2` inside the had
`p.1`. -/
...
/-- B1-B — CONFIRMS 2-03 §5.3's Forcing Conjecture for (B): **fails by collapse.**
...
/-- B1-Bs — CONFIRMS 2-03 §5.3's prediction for (B)-strict: **fails by emptiness** —
...
/-- B1-C — CONFIRMS 2-03 §5.3's Tower Conjecture: (C) **passes** B1. Over the One the
...
/-- Candidate (A2), loss-mandatory / grasp-optional (2-03 §5.1(A2)): `p.1 = H` (grasp,
may be `∅`), `p.2 = M` (remainder, mandatory nonempty). -/
def AttA2 (X : Type) : Type :=
  { p : Set (Sym2 X) × Set (Sym2 X) // p.1.Finite ∧ p.2.Finite ∧ p.2.Nonempty }

/-- B1-A2 — CONFIRMS 2-03 §5.3's prediction: (A2) **passes** B1 — but **powered by ∅.**
...
Per the wantability hazard (2-03 §5.4): (C) is the owner's original axiom returning, so
...
design are OPEN (2-03 O-2-03-1b); this structure exists only to run the anti-Mirror
...
/-- B2-C — CONFIRMS 2-03 §5.3/§5.4: (C) admits two states separated by a depth-1
...
/-
================================================================================
2-03 §5.3 predictions ↔ outcomes  (see 2-03 §5.6 Results for the prose transcript)
================================================================================
...
--
-- Deviations from 2-03-mechanization-b.md:
