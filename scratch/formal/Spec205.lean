/-
Spec 2.05 — the D19 formal pack, S1_ω, and the closing theorems of Series 2.

Normative source: `scratch/spec/2-05.md` (§1 D19, §2 D25, §5 the formal content, §6 frozen
predictions) and the work order `2-05-mechanization-a.md`. Continues `Spec204g.lean` (the whole
corrected-universe chain). Specs win.

STANDING DISCIPLINE (2-05-mechanization-a): the D19 register is now written and citable, but
...
D25 GLOSSARY (the verb emendation, doc-side; no proof term or declaration renames — 2-05 §2).
...
them under acquaintance, not spectation:
  ...
  * `HasObjEndpointΩ`, `ApproxW`, etc. — knowing-local structure. The mathematics is unmoved; the
    interpretation moves truer. The 2-00 prose sweep (A8's wording, §2.4 headers, D10) is spec-side.
-/
import Spec204g
...
/-! ## Stage 1 — the D19 formal pack (2-05 §5; the naming's machine-checked content) -/

/-- **V1 (the vanishing)** — 2-05 §5, FP-D1. At saturation the set-remainder is EMPTY: when the
...
/-- **P-D19-1 (inexhaustibility / the fog, as theorem)** — 2-05 §5, FP-D1. The tower of self-
...
/-- **P-D19-2 (privacy / inaccessibility)** — 2-05 §5, FP-D2. The residue appears in no knowing's
...
/-- **P-D19-3 (non-transmissibility)** — 2-05 §5, FP-D2. No hearsay chain assembles another's
...
/-- **T19 (the transcendental theorem)** — 2-05 §5, FP-D3. The Mirror and the Forcing Lemma,
...
/-! ## Stage 2 — S1 at the ω-tier (2-05 §5, FP-D4) -/
