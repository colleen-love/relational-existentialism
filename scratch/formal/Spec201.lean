/-
Spec 2.01 — mechanization of the two-sorted signature, enlargement, the C3 split, and L1.

Normative sources: `scratch/spec/2-00.md` (ontology) and `scratch/spec/2-01.md`
(signature and definitions). This file realizes the *two-sorted* signature of spec
2-01, the Arena's successor. `Spec200.lean` stays untouched and remains valid.

Deliverables (Spec 2-01, work order 2-01-mechanization.md §1):
...
Per Spec 2-00 D15 (pre-registration), the C3 case split is *defined first*, then the
...
definitional. Aperture comparison is inclusion within one object only (2-00 A3).
...
/-! ### The C3 split — ORDER OF DECLARATIONS IS NORMATIVE (2-00 D15)
...
/-! ## Part B — L1: the covariety closures (spec 2-01 §4)
...
/-- Spec 2-01 §4, L1 (sum, SHOULD): coherence is preserved under binary coproducts,
...
--
-- Deviations from 2-01-mechanization.md:
--   * File registered as a third Lake library root (`Spec201`) alongside
--     `Paper1`/`Spec200`, in `lake/lakefile.toml` — same second-root precedent.
