/-
Spec 2.04a — The Corrected Functor: F_C, the coherence triple, and the coherent census.

Normative source: `scratch/spec/2-04.md` (the corrected-functor design: §0 D20, §1 D22 the bare
(C) signature, §2 D23 the coherence triple, §7 the frozen FP predictions) and the work order
`scratch/spec/2-04-mechanization-a.md`. Specs win; discrepancies reported below.

NAMING DISCREPANCY (reported, resolved spec-first). The work order names its target
`Spec205.lean` and cites "2-05 §0/§2/§6" and appends results to "2-05" — but the normative spec
is `2-04.md` (its §0/§2/§6/§7 are exactly what the order cites), and the file-naming convention
is spec `2-0N` → `Spec20N` (2-03 → `Spec203`/`Spec203b`/`Spec203c`). The "2-05"/`Spec205`
references are a systematic off-by-one slip. Per "specs win," this file is `Spec204.lean` and all
citations are to `2-04.md`. The `2-05` open-question labels the spec itself uses (O-2-05-2/3/5)
...
session effort. Nothing here consults 2-04 §8 (the D19 register); FP4's hostile framing is the
...
/-! ## Stage 0 — D20 discharged in-file (2-04 §0; nothing precedes this)
...
(2-04 §0). Inhabited by the whole pattern. -/
...
/-! ### FP1b — the relation side over the point is exactly three (2-04 §0)
...
/-! ## Stage 1 predicates, pulled forward for the census (2-04 §2, D23 the coherence triple)
...
/-- K1 — endpoint coherence (2-04 §2), carried verbatim from the two-sorted signature: if an
...
/-- K2 — downward closure (2-04 §2): if `s` is part of `x` and a *relation* `r` is an endpoint
...
/-- K3 — context-nonemptiness (2-04 §2; the old OPEN, now imposed as structure): every held
...
/-- The coherence triple (2-04 §2, D23). A coherent coalgebra of the corrected functor. -/
def CoherentC (A : RawC) : Prop := K1 A ∧ K2 A ∧ K3 A

/-! ### FP1c — the coherent census (2-04 §0)
...
merely turn; it must turn upon its turning** (2-04 §0). -/
...
turn is unformulable (2-04 §0, the D4 pattern). The One cannot merely turn. -/
...
/-! ## Stage 1 — homomorphisms, L1-C (the covariety closures), and the holding lemma (2-04 §2)

The K-triple must be a coequation so the covariety/transfer machinery of 2-01 §4 applies verbatim
(2-04 §2). FP2 = each clause closed under surjective homs, injective homs, and coproducts. -/
...
and the 2-01 §4 transfer machinery applies verbatim (2-04 §2). -/
...
/-- The holding lemma — **"to bear is to hold"** (2-04 §2). K1 and K2 compose on a mixed tie: if
...
part of her pattern — the shared-joint surplus of 2-00 §2.5, the per-act mechanism behind
...
/-
================================================================================
2-04 §7 FP predictions ↔ this file (Stages 0–1; see 2-04 §9 Results for the transcript)
================================================================================
...
--
-- Naming: file = Spec204.lean (spec is 2-04.md; the order's "Spec205"/"2-05" §0/§2/§6 are an
