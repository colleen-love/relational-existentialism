...
Category-theory machinery is hand-rolled deliberately (Spec 2.00 §2 treats the
...
/-- A coalgebra of `F`: a carrier with an unfolding map. Spec 2.00: a "description"
...
/-- Spec 2.00, T1b corollary. Every state is bisimilar to every state: the many do
...
/-- Spec 2.00, T1c. The collapse is about *all* final coalgebras, not just `unit`:
...
(Spec 2.00, O1). They are unblocked because they depend only on the settled *shape* of
...
the interface*. Spec 2.01 will instantiate `Arena`; these theorems then apply to the
instance automatically. Two honest flags carried in the doc-comments below:
(1) these are interface-relative results — fully discharged only when 2.01 provides the
...
deliberately NOT proved here (Spec 2.00 §7 out-of-scope; P1 general deferred to 2.01). -/
...
/-! ## Axiom audit (Spec 2.00 §6.1, item 3)
...
/-
-- Spec 2.00 ↔ scratch/formal/Spec200.lean
...
--
-- Deviations from 2.00-mechanization.md:
--   * File lives at `scratch/formal/Spec200.lean`, not `formal/Spec200.lean`.
--     Reason: after the handoff-XVIII rename the repo's `/formal/` is a gitignored
--     stale build home; the live Lean source root is `scratch/formal` (Lake library
--     `Paper1`, per `lake/lakefile.toml`). Placing the file under `/formal/` would
