/-
`series-5/formal/ws1.lean`

WS1 — **The tower and its colimit.** Series 5, blocking workstream.

This file is SELF-CONTAINED (charter §1, `spec/readme.md`): it carries its own copy
of the Series 4 carrier machinery — the plain κ-bounded powerset carrier `νPk`
(from Series 4 `ws1`), the labelled carrier `νLk` and its composition `lcomp` (from
Series 4 `ws3`), and the carrier lower bound (from Series 4 `ws2`) — re-namespaced
`Series5.WS1`. Nothing is imported from `series-4/` or `archive/`.

The *new* Series 5 content (from `Level` onward) is the **directed tower** `Tower`,
...
Design doc: `series-5/spec/ws01-design.md` (candidate C2, bound-relaxing inclusions).
...
/-! ## The κ-bounded powerset functor `P_κ` (transcribed Series 4 WS1) -/
...
/-! ## Carrier lower bound (transcribed Series 4 WS2 `carrier_card_ge`) -/
...
/-! ## Reachability and the face (transcribed Series 4 WS1) -/
...
/-! ## The labelled (faced) carrier `νLk κ Q` (transcribed Series 4 WS3) -/
...
/-! ## Composition stays atom-free (transcribed Series 4 WS3 `lcomp`) -/
...
"the gate is level-local `bisim_eq` + `ι_inj`"). Series 4 never needed a `bisim_eq` for
the labelled carrier `νLk`; Series 5's gate does, so we derive it directly from
