# Design + Executor Instructions: Closing Series 5 Open Obligations #2 and #12

**Owner workstream:** WS6 (with a WS3/WS1 carrier touch for #2).
**Status of this document:** a design contract, written to the Series 5 protocol.
...
1. **The collapse lives on the *plain, unlabelled* carrier `νPk κ`.** Its bisimulation
   ...
   bisimilar. This is precisely the Series 4 lesson (`ws3_loopface_ne`,
...
- The Series 5 payoff tracker row "Groundlessness without collapse" updated: the
...
1. **Find `lcomp` and its destructor law.** In Series 4 WS3, `lcomp` is the labelled
   composition operator and `lcomp_dest` is its computation rule (it appears in the S4
   axiom log as `Series4.WS3.lcomp_dest`). Series 5 transcribes the faced carrier;
...
- The Series 5 payoff tracker "Cross-level relating, leak-free" coincidence column
...
Follow the Series 5 protocol phases for a scoped reopen, not a full series run:
...
5. **Regenerate the axiom log against a clean build**, and update the README theorem
   counts (Series 5 goes from 43 to 43 + the new headline theorems).

## What must stay true no matter what (invariants)

- Sorry-free, no custom axioms, standard three only.
- Self-contained: nothing imported from `archive/` or `series-4/`; the faced carrier
  and `lcomp` are transcribed into Series 5 as they already are.
