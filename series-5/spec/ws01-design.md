...
**Design doc. Series 5, blocking workstream. Owns: the directed system of faced carriers `W_α`, the connecting maps `ι_{α,β}`, the colimit carrier `W_∞`, the coalgebra-structure gate on which the whole program is conditional, and the recovery of Ω inside `W_∞`.**

*Series 5 is standalone. It carries its own copy of every Series 4 carrier lemma it needs (charter §1: "it is ok to copy theorems from 4 into 5"). The Series 4 names reproduced here — `PkObj`, `PkMap`, `Coalg`, `IsTerminalCoalg`, `lambek`, `Bisim`, `bisim_eq`, `νPk`, `νPk_terminal`, `Reaches`, `ReachSet`, `face`, `LkObj`, `LkMap`, `νLk`, `lstr`, `Cofix.corec`, `Cofix.dest`, `qpfLk`, `carrier_card_ge` — are transcribed into `series-5/formal/ws1.lean` verbatim from `series-4/formal/{ws1,ws3}.lean` and re-namespaced `Series5.WS1`. Nothing is imported from `series-4/` or `archive/`.*

## The object at stake

Series 4 built a single faced carrier: `νLk κ Q = Cofix (X ↦ P_κ(Q × X))`, the terminal coalgebra of the labelled powerset functor (`series-4/formal/ws3.lean`). That carrier has one fixed cardinal `κ` and one fixed label set `Q`; the Explosion Dilemma (WS2) proves any *single* such carrier is either capped by fiat or collapsed. Series 5's carrier is not a coalgebra but a **directed system** of them,

> `α ↦ W_α`, with `W_α` a Series-4-style faced carrier at cardinal `κ_α`, connected by `ι_{α,β} : W_α → W_β` for `α ≤ β`,

and the world is the colimit `W_∞ = colim_α W_α`. The design question is which shape of directed system (a) makes `W_∞` **carry a coalgebra** (a structure map `W_∞ → F(W_∞)` for the appropriate `F`), (b) keeps **bisimulation-is-identity** on `W_∞` (charter point 1 / criterion iii — the analogue of Series 4's weak-pullback gate), and (c) recovers **Ω = {Ω}** as a genuine inhabitant, so the tower refines rather than abandons the Series 4 object.
...
-- face, carrier_card_ge, bisim_eq) transcribed verbatim from Series 4; see header.

variable {Q : Type u}

/-- A **level**: a Series-4 faced carrier at a cardinal `κ_α ≥ ℵ₀`. -/
...
theorem ws1_bisim_eq_colim (T : Tower Q)
    ...
    ∀ x y, R x y → x = y
```
*Strategy:* `destInf` is well-defined on the quotient because `ι_dest` says the connecting maps commute with `dest`, so the level-α unfolding of a representative agrees (after reindexing) with the level-β unfolding of any equivalent representative — this is the one place `ι_dest` is load-bearing. For `ws1_bisim_eq_colim`: a bisimulation `R` on `W_∞` pulls back along `toColim_a` to a bisimulation on each `W_α = νLk κ_α Q`; by the level-local `bisim_eq` (transcribed Series 4 fact) each pullback is contained in the diagonal; because `ι_inj` the diagonals cohere, so `R` is the colimit diagonal. **The gate is thus: level-local `bisim_eq` + injectivity of connecting maps.** *Paper-decidable:* yes — reduces to a proved Series 4 theorem applied levelwise plus injectivity, no new coinduction.
...
noncomputable def omegaInf (T : Tower Q) (q : Q) : Winf T :=
  toColim T (Series5.WS1.loopState (a := T.someIdx) q (T.lvl _).hinf)
