/-
Spec 2.4b — Continuation of 2-4-mechanization-a: the univariate reduction (Stage 2),
...
/-! ## Stage 2 (reduction) — the mutual functor collapses to one univariate fixpoint (2.4 §4)
...
/-! ## Stage 5 (FP4) — the residue, attacked; the R-C3 split (2.4 §3, §7)
...
/-- Direction/context of `r` at host `x` (2.4 §3, D24): the relations in `x`'s pattern that bear
...
`Ω_C` (that question stays open honestly, 2.4 O-2-5-2, pending the construction). The saturation
...
/-! ## Stage 6 — the B3 recovery pack and B5 conservativity (2.4 §3, §6)
...
/-- Reveal (2.4 §3): the relation together with its context. The threefold payload (D17) reads
off this unchanged from 2.2 §3. -/
...
/-- Containment of an object in another: pattern-inclusion (the ratified parthood, 2.2 §6). -/
def ContainedInC (A : RawC) (y x : A.O) : Prop := A.pat y ⊆ A.pat x

/-- An endpoint (of either sort) contained in `x` (2.4 §3): an object endpoint by
...
/-- Internal relations of `x`: those held entirely among contained parts (2.4 §3 / 2.2 §6). The
...
/-- A windowless object: no boundary at all (2.2 §6; C1 conjectures these impossible in a
...
/-- π — the ratified containment overlap (2.4 §6, R1 untouched): the shared relations of two
objects. -/
...
/-! ### FP7 — the witness model `boolWitnessC` (2.4 §6)
...
K-triple neither over- nor under-constrains (2.4 §7). -/
...
/-! ## Stage 7 (FP3) — P3h and the hosted-relations doctrine (2.4 §5)
...
/-- A relation is *hosted* when it lies in some object's pattern (2.4 §5). Definable, as the
doctrine requires. -/
...
behaviors no object picks up (2.4 §5). The doctrine of §5 (the ontology's relations are the
hosted ones) is thereby necessary, not decorative. The `Ω_C`-proper form of FP3 (an unhosted
point *inside* Ω_C) awaits the construction (2.4 O-2-5-*); this records the phenomenon at the
interface. -/
...
/-
================================================================================
2.4 §7 FP predictions ↔ this file (Stages 2r,4,5,6,7; see 2.4 §9 Results)
================================================================================
...
--
-- Marked hostile-first deviation (sanctioned, 2-4-mechanization-a §3): Stages 2-3's mutual
