...
Run gate B1 (the F(1)/Parmenides check, D20) on every entrant, symmetrically; prove the Forcing Lemma at whatever generality closes; run the B2 anti-Mirror spot-check on candidate (C) **before any (C)-development** (2.3 §5.4: the wanted candidate is scrutinized first).
...
`B1_Bs_empty`'s doc-comment carries its name from 2.3 §5.3: **the arrow cannot turn** — austere properness forbids the One from attending at all; a universe built on strict inclusion alone is *empty* at the fiber where T1 demands the self-loop. Both B-failure-modes (collapse and emptiness) are thereby on record.
...
**B1-A2 (the evasion-clause entrant, 2.3 §5.1/§5.3).**
...
parthood design are OPEN (2.3 O-2-3-1b); this structure exists only to run
...
```
-- 2.3 §5.3 predictions ↔ outcomes (fill in: CONFIRMED / REFUTED)
