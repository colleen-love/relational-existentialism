...
3. **ASCENT** — O-2-3-2's first two sufficiency checks for (C): the collapse argument fails at *every* stage of the terminal sequence, not just the first — plus the honesty lemma that at depth 1 the pass is carried entirely by the relation sort (the object sort over the point is still a point until stage 2).
...
**The honest work here is the statement, as it was for FL.** Budget a day. The frozen conjecture (2.3 §5.3 C-B6): inside (C), aperture = higher-order neighborhood, remainder = its complement. Two readings, both mechanized:
...
## 5. ASCENT — stage-wise non-collapse and growth (O-2-3-2, first two checks)
