...
Spec 2.0 identifies exactly one theorem group as *unblocked* — provable now, independent of all open machinery (the enriched/quantaloid setting of O1 is NOT needed):
...
- **Doc-comments:** every definition and theorem gets a `/-- ... -/` doc-comment containing (a) the spec ID (e.g., "Spec 2.0, T1a"), (b) one sentence of mathematical content, (c) one sentence of philosophical content, drawn from this document.
...
/-- Spec 2.0, T1b corollary. Every state is bisimilar to every state:
...
-- Deviations from 2.0-mechanization.md: <list here, or "none">
```
