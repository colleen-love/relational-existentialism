...
**Out of scope:** the corrected functor (awaits O-2-3-1 — the owner's fork ruling); anything that would *repair* rather than *certify* (no new functors, no candidate F(1) computations — those are pre-registered deliverables of the correction spec, D20); the S1 pigeonhole; T4/T7.

**Tone requirement for doc-comments:** this file records the project's machinery catching the project's authors. Write it with the confidence that deserves — the Mirror is a *result*, not an embarrassment. The 2.3 §0 framing ("T1 firing on our own construction"; "the boolean shadow in two-sorted clothing") should appear nearly verbatim in `omega_collapse`'s comment.
...
```lean
/-- Spec 2.3 D20 — THE F(1) DIAGNOSIS. Over a one-point world there is exactly
...
```lean
/-- Spec 2.3 T17 — THE MIRROR. The constructed universe is a single point:
...
The many require loss in the functor (2.3 D21). This is the hostile-first
...
/-- Spec 2.3 T18 — THE COINCIDENCE OF EXTREMES. In the collapsed universe the
...
```lean
/-- HONESTY LEMMA (Spec 2.3 §1). Genesis is VACUOUSLY TRUE over the collapsed
...
against the corrected Ω (2.3 §5). Kept in the file so no later reader
