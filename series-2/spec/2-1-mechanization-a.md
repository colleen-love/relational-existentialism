...
Per 2.0 D15, the split is **defined first**, then the generic theorem, then the counterexample. Do not reorder.
...
1. **Do not add a properness axiom to `Model` to "complete" the picture.** The whole point is to see what the signature forces without A8(i). The axiom's formal placement is a 2.1/2.2 design decision, not yours.
...
-- Deviations from 2-1-mechanization.md: <list here, or "none">
```
