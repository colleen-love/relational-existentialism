...
* `Paper1.Arena` — the relational arena (relations primitive; converse; composition; the diagonal
  `reenter S = S ⨾ S`, i.e. **D1, self-relation as the trace**). No axioms, no self named.
* `Paper1.Arrow` — **the headline theorem:** self-relation is **irreversible** (`reenter` is non-injective —
  `S` is unrecoverable from `S ⨾ S`; a semigroup, not a group). `spec/02-foundation.md`.
