...
- **Accumulation** `[proved]` — relating only *grows* the attention field, never depletes it:
  [`Attention.orbit_ascending`, `orbit_le_gfp`](../../formal/Archive/Scratch/Attention.lean), with
...
- **Closing = the self** `[proved/structural]` — the eigenform is the fixed point of the
  ...
  close (sparsity, [03.7](../archive/03.7-sparsity.md)), so creation is the rare case. `[follows]`
...
eigenform ([the distributed self](../archive/03.6-the-self-quantified.md)) `[reading]` — but it forecloses
...
proved `[proved]`: [`Attention.relating_absorbs` / `closed_loop_registers`](../../formal/Archive/Scratch/Attention.lean)
...
`no_complete_view` consumes — is mechanized in [`Registration.lean`](../../formal/Archive/Scratch/Registration.lean).
...
cartesian diagonal here. [`Knowing.lean`](../../formal/Archive/Scratch/Knowing.lean) ties them on one axis.
