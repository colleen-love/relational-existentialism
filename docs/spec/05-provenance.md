...
| **Non-cartesian reflexive object** | A *linear/compact* reflexive object (GoI / `Int`-construction, `D ≅ D* ⊗ D`) where `Y` is the **trace** with no cartesian copy. | `SelfApplication`, `GraphModel`, `Compact`, `IntConstruction`, `RelCompact` | **SNAKE EQUATIONS CLOSED (concretely); abstract `Int(C)` + reflexive object open.** `SelfApplication` is the cartesian witness; `IntConstruction` builds the GoI `Int(C)` on any traced SMC — objects, two-way hom, object **dual** (involution / monoidal / unit), identity, and the **morphism-level dual functor** `IntDualHom` (involutive, identity-preserving, 0 axioms). **`RelCompact`: `Rel` is compact closed with both zigzag / triangle (snake) identities proved `= id`** (`rel_snake_right`/`left`, `aesop`) plus the `Compact.CompactClosed` name bijection (`relCompactClosed`) — the compact-closed **snake axioms** discharged in the canonical model. What remains research-grade: the **abstract `Int(C)` composition-via-trace** for arbitrary non-strict `C` (the full JSV/AHS chase), the dual's full functoriality, and the **linear reflexive object** inside the compact arena. Construction side. |
