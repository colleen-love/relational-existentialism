...
| Cartesian trace ⟺ Conway fixed-point operator | Hasegawa; Hyland; Bloom–Ésik (iteration theories) | **R** | `ConwayTrace`: the operator and its identities — `pfp`, the **rolling/dinaturality** rule (not in mathlib), the Bekić **diagonal** rule — are built (§4); the full `TracedSMC` *instance* packaging is the flagged remainder. |
...
closed (the latter as a proved boundary); the **Conway operator** and the **`Int`-construction arena**
built at their mathematical cores, with only the categorical packaging / law-verification flagged. The
one genuinely-open target is the **conjecture lift**.
...
| **Conway-trace instance** | The **fixpoint**-trace on domains (the Hasegawa cartesian-trace = Conway-operator correspondence). | `ConwayTrace`, `DomainTraced`, `GraphModel` | **OPERATOR DONE; packaging open.** `ConwayTrace` builds the Conway operator and its load-bearing identities — `pfp` (fixpoint/least/parameter-naturality), the **rolling/dinaturality** rule (not in mathlib) and the Bekić **diagonal** rule. What remains is the **categorical wrapper**: discharging the seven JSV axioms into a multi-object `TracedSMC` instance through the product/projection bookkeeping. Construction side (orthogonal to the seam). |
