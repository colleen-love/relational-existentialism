...
| **Non-cartesian reflexive object** | A *linear/compact* reflexive object (GoI / `Int`-construction, `D ≅ D* ⊗ D`) where `Y` is the **trace** with no cartesian copy. | `SelfApplication`, `GraphModel`, `Compact`, `IntConstruction` | **CORE + DUAL-ON-MORPHISMS DONE; full compact closure open.** `SelfApplication` is the cartesian witness; `IntConstruction` builds the GoI `Int(C)` on any traced SMC — objects, the two-way hom, the object **dual** (involution / monoidal / unit), the identity, **and the morphism-level dual functor** `IntDualHom` (the contravariant transpose), proved identity-preserving (`IntDualHom_id`) and **involutive** (`IntDualHom_involutive`, `(fᵈ)ᵈ = f`) over a coherent traced SMC, both **0 axioms** (the swap-braids cancel by symmetry). The non-cartesian setting where feedback replaces copy. What remains research-grade: **composition via the trace** and the **compact-closed/snake axioms** from JSV (and with them, the dual's full functoriality). Construction side (constructs `Y`, by the duality). |
...
| **The conjecture lift** | Sparsity for **all of `Cl(𝕋)`** (Conjecture 3.3) and the spectral/closure form (Conjecture 3.4). | [`03.1 §3.4`, `§3.6`](03.1-sparsity.md), `Peripheral` | **PARTLY CLOSED; general form open.** The reconciliation is **decided** ([03.1 §3.6](03.1-sparsity.md)): one generator `L`, `Φ_c` its sustaining flow, `E` its peripheral spectral projection ∘ seam-orientation (*not* `lim Φ_c`), the weight `Φ_c`'s invariant state (relational, canonical iff primitive). The **spectral picture of `E`** is mechanized `sorry`-free (`Peripheral`, finite dim): `E = dephase` idempotent ⇒ spectrum `⊆ {0,1}` (`dephase_eigenvalue`), **veto-check: no rotating peripheral spectrum** (`dephase_no_rotating_peripheral`, peripheral = fixed = classical), invariant trace. **Open:** the general finite-dim Perron–Frobenius for an arbitrary unital CP `Φ_c` (peripheral subalgebra possibly non-commutative / rotating) and the infinite-dim standard form; the cost-graded sparsity for all of `Cl(𝕋)`. The deepest remaining target. |
