...
3. **Connect the threshold to the fixed point.** Prove
   ...
   **◑ the abstract `σ`, now discharged for the depth** —
   ...
   — now closed for the **depth** (the load-bearing quantity). What remains: a *forced*, non-trivial
   floor `d ≥ 2`, the valuation `μ` (numeric cost), and finite-depth convergence — all now phrased
   about `Φ_c` itself, not an abstract endomap. That residue remains **open**.
