...
> floor is supplied as a hypothesis, tracing back to the depth posit `d ≥ 2` (§3.5 step 3), which is
> itself not derived. So Lemmas 3.1/3.2 prove the *trivial half* — given the model, the count is
> rationed; the **structural** rarity is §3.4's nowhere-dense result.
...
3. **Connect the threshold to the fixed point.** Prove
   ...
   **discharged for the real operator**; the residue is now a single standard order condition — **ACC
   ...
   What remains genuinely open: a *forced*, non-trivial floor `d ≥ 2`, and the valuation `μ` for the
   numeric cost. Convergence as such is no longer open.
