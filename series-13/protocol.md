...
2. **Never relabel a shortfall as the goal, and reach for the pre-registered outcome BEFORE the compiler.** When a build falls short, the honest outcome is a **Partial** (or **Total**, or **Disconnected**, or **Refuted**, or **Circular** for the audit) with the obstruction made precise, never a quiet redefinition of the target. The charter is the fixed bar; `charter-status.md` records the miss. The charter's pre-registered alternatives (Total in its direction, Disconnected with the WS1 obstruction, the Partial clauses) are HONORABLE, anticipated moves. When a finding shows a pre-registered condition has been met, reporting it is correct; constructing a theorem that avoids it, OR one that manufactures the expected DUAL terminus past a real obstruction, is the §0.2 sin.
...
When every workstream has reached a terminal state and a review pass returns no SERIOUS findings, Series 13 closes. Because Series 13 is a coda and NOT a new terminus, Phase F does not rewrite the program-level synthesis; it writes `summary.md` and `summary-technical.md` for the coda and adds ONE row to the root `README.md` series table (the fit, and which terminus was reached, Dual / Total / Disconnected / Partial), explicitly framed as a post-terminal sharpening that leaves Series 12's completion intact. If DUAL landed, the close states the one new permanent open, the classification of the out-of-image imports, open as a theorem by the same result that locates them. The coda does not resolve to a new key; it shows the two hands were playing one figure an octave apart, and lets the last interval stand open.
...
- **WS5 owns the verdict and consumes WS1-WS4.** The verdict is a FUNCTION of the orders' non-triviality, the transport-with-exogeneity, the connection's genuine defect of round trip, and the mint's non-surjectivity (or Total), never hand-set; it cannot be computed until WS4 settles. WS5 also folds in the audit (the fork-open, genuine-connection, exogeneity, structural-defect, strip, and names-not-terms checks) and reports the terminus, and the coda's one new open if DUAL.
...
## 6. End of series, and the coda's place

When every workstream has reached a terminal state and a review pass returns no SERIOUS findings, Series 13 closes. Because Series 13 is a coda after the program's genuine terminus, there is no successor in the ordinary sense, and Phase F does not rewrite the program synthesis; it records the fit and leaves Series 12's completion intact. Should the coda itself surface a hard resistance worth a successor (for instance, a Disconnected finding whose obstruction a differently-ordered attempt could dissolve, or a Dual whose out-of-image imports a richer inspection type could probe without sorting them), the inter-series discipline still applies and a successor could be chartered as a response to that finding:

> **Each series is a response to the previous series' honest findings, not a continuation of its plan.**

But the coda's intent is to name the fit and stop, adding no name and reopening nothing. The mechanics, unchanged if a successor is chartered:

1. Freeze the coda under `archive/`. Readable as origin, normative for nothing.
