...
**Design sub-artifact. Series 13, WS1's concrete construction, consumed by WS2–WS4. Owns: the ONE preorder on inspections (`instLEInsp`) and the ONE preorder on labelled coalgebras (`instLELab`), the paper triage of each candidate against the two questions the protocol fixes (is the order non-trivial? does it admit a monotone mint?), the winning pair with its non-triviality proofs, and the rejected candidates recorded. This is the knot: the whole series rests on a non-trivial pair of orders admitting a monotone mint and a non-vacuous connection. DISCONNECTED is the pre-registered honest outcome, and this artifact records the precise obstruction that would trigger it (the variance wall).**
