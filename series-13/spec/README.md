...
/-- **The order on inspections.** By the RESIDUE: `insp₁` is below `insp₂` iff `insp₁`'s residue is
    ...
    quantity the whole coda turns on; it is the ONLY order under which the residue-broadcasting mint is
...
inductive Series13Verdict
  | Dual          -- expected terminus: orders non-trivial + transport with exogeneity + connection with a
