...
root. Its live modules are dual witnesses for [paper two](../paper-2/spec/paper-two.md). The
...
one that theorem [3.2](../paper-2/spec/T2.05-lived-identity.md) (the "we") and definition [D1](../theory/spec/AXIOMS.md)
...
Two **live** modules, both from scratch over only the standard library, witnessing **paper two**:
[`RelExist/Coinductive.agda`](RelExist/Coinductive.agda) — the ν-layer proper (theorem
[3.2](../paper-2/spec/T2.05-lived-identity.md), the lived identity `≈`) — and
[`RelExist/Inversion.agda`](RelExist/Inversion.agda) — the first-person surplus `≈ ⊊ ≅` (the
[3.5](../paper-2/spec/T2.08-decoherence.md) inversion), proved in *both* assistants (cf. Lean
...
| **3.2** — `≈` (the **lived identity**) as the greatest bisimulation | `_≈_` | [3.2](../paper-2/spec/T2.05-lived-identity.md) | ✅ defined (coinductive record) |
| `≈` is an equivalence | `≈-refl` / `≈-sym` / `≈-trans` / `≈-isEquivalence` | [3.2](../paper-2/spec/T2.05-lived-identity.md) | ✅ proved (copattern corecursion) |
| **shared world** `𝔼 := D/≈` | `SharedWorld` | [3.2](../paper-2/spec/T2.05-lived-identity.md) | ✅ defined (`Setoid`) |
| **coinduction** — every bisimulation `⊆ ≈` | `coinduction` (from `Bisimulation`) | [3.2](../paper-2/spec/T2.05-lived-identity.md) | ✅ proved (one guarded definition) |
| **observational equality** `≅` (the outside view) | `_≅_` | [02 A2](../theory/spec/AXIOMS.md), [03.5](../paper-2/spec/T2.08-decoherence.md) | ✅ defined (same observation stream) |
| **deterministic collapse** `≈ ⟺ ≅` (the *boundary* case) | `≈⇒≅` / `≅⇒≈` | [03.5](../paper-2/spec/T2.08-decoherence.md) | ✅ proved — a clockwork has *no surplus*; the gap is `RelExist.Inversion` below |
...
| --- | --- | --- | --- |
| nondeterministic transition system (the early/late witness) | `Step` (one constructor per edge) | [03.5](../paper-2/spec/T2.08-decoherence.md) | ✅ defined |
...
| **soundness** `≈ ⊆ ≅` | `≈⇒≅` | [03.5](../paper-2/spec/T2.08-decoherence.md) | ✅ proved |
| **strictness** `≈ ⊊ ≅` — the first-person surplus | `surplus` (`p0≅q0` ∧ `¬p0≈q0`) | [03.5](../paper-2/spec/T2.08-decoherence.md) | ✅ proved (matches Lean `bisim_ne_obsEq`) |
