...
Deliverables (work order 2-2-mechanization.md §1):
...
/-- Spec 2.2 §3, T13 hearsay bounds. What arrives from `z` through the shared `s`
...
/-- Spec 2.2 §3, T13: the received payload can be a PROPER part of the source's —
...
/-- Spec 2.2 §4: the internal part of a pattern — relations all of whose endpoints
...
/-- Spec 2.2 §6 (R3): the depth of x's hold on r — the internalized part of the
...
/-- Spec 2.0 §2.5, the musicianship corner: mutual partiality — each object holds a
