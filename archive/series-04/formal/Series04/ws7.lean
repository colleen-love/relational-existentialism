/-
`series-04/formal/ws7.lean`

WS7 — **The anti-trivialization audit.** Series 04, the honesty workstream. Owns the
...
theorem ws7_finitude_of_facing (hinf : ℵ₀ ≤ κ) : FinitudeOfFacing hinf :=
  ⟨fun x hx => Series04.WS6.ws6_selfface_proper_nonselfrelating x hx,
...
theorem ws7_payoffs_hold {Q : Type u} (hinf : ℵ₀ ≤ κ) {q₁ q₂ : Q} (hq : q₁ ≠ q₂) :
    -- plurality (WS3)
    (∃ a b : Series04.WS3.νLk κ Q, a ≠ b ∧ Series04.WS3.NonAtomic a ∧ Series04.WS3.NonAtomic b)
    -- no top (WS4)
  ∧ (∀ x : (νPk κ).X, ¬ ∀ y, y ∈ ((νPk κ).str x).1)
    -- positioned views (WS4)
  ∧ (∀ o : Series04.WS4.Observation κ, ∃ x y, Series04.WS4.viewOf o = face x y)
  ...
  ⟨Series04.WS3.ws3_plurality_core hinf hq,
   Series04.WS4.ws4_no_top_cardinal,
   Series04.WS4.ws4_view_is_positioned,
   Series04.WS5.ws5_omega_endogenous_point hinf,
   fun x hx => Series04.WS6.ws6_selfface_proper_nonselfrelating x hx,
   (Series04.WS6.ws6_omega_nonterminating hinf).2⟩
...
theorem ws7_plurality_vs_collapse_distinct {Q : Type u} (hinf : ℵ₀ ≤ κ) {q₁ q₂ : Q}
    (hq : q₁ ≠ q₂) :
    (∃ a b : Series04.WS3.νLk κ Q, a ≠ b)
  ∧ (∀ a b : (νPk κ).X, HereditarilyNonempty a → HereditarilyNonempty b → a = b) :=
  ⟨by obtain ⟨a, b, hab, _, _⟩ := Series04.WS3.ws3_plurality_core hinf hq; exact ⟨a, b, hab⟩,
   fun a b ha hb => Series04.WS2.ws2_collapse hinf a b ha hb⟩
