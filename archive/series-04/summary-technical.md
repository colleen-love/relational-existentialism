# Relational Existentialism — Series 04: Technical Status Summary
...
Series 04 is **completely self-contained**: the carrier machinery reused from the archived Series 03 development is transcribed into `series-04/formal/`, and nothing is imported from `archive/`.
...
- **In-build verification, machine-run:** `AxiomCheck.lean` imports the whole build (`Series04.lean`) and runs `#print axioms` on **41 headline theorems** across WS1–WS7. The captured run is committed at [`spec/axiom-check-log.md`](./spec/axiom-check-log.md) against **Lean 4 v4.15.0 / Mathlib v4.15.0**. A publication should cite the specific commit hash and a clean-build log.
- **Closure:** `scripts/gate.sh` confirms the `series-04/formal/` imports resolve only to Series 04's own roots plus Mathlib — no cross-series leakage.
