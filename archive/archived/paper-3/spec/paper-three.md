# Paper three — self-relation is modular flow, and time's two faces are one generator

> **Renumbering note (handoff I.O).** The papers were rotated back one slot: a new foundational
> *paper one* (*a self arises from the axioms*) goes at the front, the old paper one (time/arrow/energy/seam)
> is now **paper two**, and this paper is now **paper three**. Structural identifiers (library `Paper3`, the
> `T3.x` nodes, file paths) are updated; the **in-essay paper-number cross-references below** ("paper one",
> "paper two") are **not yet renumbered** — that narrative pass is deferred to the formalization pass. Read
> "paper one" in the body as the arrow paper (now paper two).
...
the frozen fork [`paper-3/formal/`](../formal) (library `Paper3`, forked from `theory/` at
...
→ [`Paper3.OneGenerator`](../formal/Paper3/OneGenerator.lean).
...
→ [`Paper3.Einselection`](../formal/Paper3/Einselection.lean).
...
one"**, and not forked. So `paper-3/` imports no `paper-2/` path; the citation carries the meaning (see
