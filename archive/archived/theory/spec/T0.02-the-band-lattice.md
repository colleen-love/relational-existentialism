# T0.02 — The band lattice

> *Node `T0.02`.* Lean: `Theory.BandCoincidence` (`theory/formal/Theory/BandCoincidence.lean`); imports
> **T0.01** (`Theory.RotatingSpectrum`). **Double-imported**, a shared `T` node.
...
lattice on which `T0.03` proves the band coincidence (the conserved remainder *is* the energy band) from the
axioms.

Consumed by **T0.03** (`BandFromAxioms`). Canonical axioms: [`AXIOMS.md`](AXIOMS.md).
