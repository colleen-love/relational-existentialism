# T0.05 — Lived-identity coinduction

> *Node `T0.05`.* Lean: `Theory.We` (`theory/formal/Theory/We.lean`); imports mathlib only.
...
is the coinductive base the A2-priority result (`T0.06`) descends to the world of selves `𝔼 = D/≈`.

*(Paper one carries its own lived-identity node, [`T2.05-lived-identity.md`](../../paper-2/spec/T2.05-lived-identity.md),
for theorem 3.2; `Theory.We` is the copy `theory/`'s `Priority`/`Axioms` need, since `theory/` cannot import a
paper.)*

Consumed by **T0.06** (`Priority`). Canonical axioms: [`AXIOMS.md`](AXIOMS.md).
