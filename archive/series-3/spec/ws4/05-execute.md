...
Now let me append the Tier 2 block (functor + QPF + terminal + Lambek + bisim). I'll replace the final `end Series3.WS4`.
...
Now let me import ws4 in Series3.lean and build the whole project.
...
**Merged `main`** (fast-forward), wrote the Lean 4 proof in `series-3/formal/ws4.lean`, imported it in `Series3.lean`, added `"ws4"` to the lakefile roots. **Full build compiles.**
