...
The module `ws1` isn't registered as part of the library (only `Series3` is a root). I need to register it so Lake builds it. Let me add it to the library roots.
...
**`series-3/formal/ws1.lean`** — formalizes the WS1 design (`4-charter-design-review.md`): the Groundless Carrier as the terminal coalgebra of the κ-bounded powerset functor `P_κ`, and everything the design's dependency DAG (§7) builds on it:
