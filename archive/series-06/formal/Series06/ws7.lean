/-
`series-06/formal/ws7.lean`

WS7 — **The anti-trivialization audit.** Series 06, runs last; emits the typed verdict.
...
Verdict: `verdict false false true = payoffsEstablished`. The central Series 06 finding is a
...
open Series06.WS1 Series06.WS3 Series06.WS4 Series06.WS6 Cardinal

variable {κ : Cardinal.{u}}

/-! ## The typed program verdict -/

/-- Series 06's three outcomes. -/
...
theorem ws7_payoffs_hold (_hinf : ℵ₀ ≤ κ) :
  ...
  ∧ (∀ S : Series06.WS2.Static κ, Series06.WS2.HereditarilyAtomless S → Subsingleton S.X)
  ∧ ((∀ a b : Proc κ, Series06.WS5.Agree a b) ↔ Subsingleton (Proc κ)) :=
  ⟨ws3_incompleteness,
   fun m => ws6_groundlessness_from_diagonal m,
   fun S h => Series06.WS2.ws2_static_collapse S ⟨h, S.behav⟩,
   Series06.WS5.ws5_agreement_is_collapse⟩
...
theorem ws7_distinctness_anchors (hinf : ℵ₀ ≤ κ) :
  ...
  ⟨⟨ws1_omega_process hinf, ws6_atomless_and_plural_impossible hinf⟩,
   ...
   Series06.WS5.ws5_incomparability_is_bare_poset hinf⟩
...
/-- **`ws7_c2_collapses` — the central Series 06 finding (Impossibility, a success).** The naive
