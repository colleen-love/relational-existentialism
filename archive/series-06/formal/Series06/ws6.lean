/-
`series-06/formal/ws6.lean`

WS6 — **Globally atomless and plural, and the one-engine unification.** Series 06, headline.
...
theorem ws6_plurality_costs_an_atom (hinf : ℵ₀ ≤ κ) :
    (∃ x y : Proc κ, x ≠ y)
  ∧ ¬ Productive (Series06.WS5.emptyProc hinf) := by
  refine ⟨Series06.WS5.ws5_processes_plural hinf, ?_⟩
  intro hprod
  exact (ws6_atomless_and_plural_impossible hinf)
    ⟨omegaProc hinf, Series06.WS5.emptyProc hinf, ws1_omega_process hinf, hprod,
     Series06.WS5.omega_ne_empty hinf⟩
...
theorem ws6_one_engine_obstructed (hinf : ℵ₀ ≤ κ) :
  ...
  ⟨fun m => ws6_groundlessness_from_diagonal m,
   Series06.WS5.ws5_processes_plural hinf,
