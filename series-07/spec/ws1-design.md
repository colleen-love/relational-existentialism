...
**Design doc. Series 07, the blocking engine. Owns: the general lemma that on *any* plain `P_κ`-coalgebra all hereditarily-non-empty states are bisimilar, and the recovery of the program's two existing collapses (static, dynamic) as its instances.**

*Series 07 is standalone; all names are transcribed into `series-07/formal/Series07/ws1.lean` and re-namespaced `Series07.WS1` (see `spec/README.md` §6). The core of this workstream is **already built** in Series 06 `ws2.lean` (`hneRel`, `hneRel_isBisim`); WS1's job is to first-class it as the named engine and connect it to both instances. Shared predicates are defined in `spec/README.md` §2 and cited here.*

## The object at stake

The charter's hinge (§3): a single lemma is the root of every collapse the program has proved. Series 04's Parmenides collapse (`ws2_collapse`) concluded *equality* on the terminal coalgebra; Series 06's process collapse (`ws1_productive_unique`) concluded uniqueness on the founded approximations. Both are the same fact one step earlier: **atomless states have one behavior.** WS1 must state that fact at its natural generality — *bisimilarity*, on an *arbitrary* plain coalgebra, not assuming terminality — because it is the weaker, more general form (bisimilarity, not equality) that reaches beyond the terminal object to any construction faithful to relating. Equality is then WS2's job (add behavioral identity). The design question is only how to state and prove the bisimilarity lemma so that (a) it is genuinely general (any `dest`, not just `νPk`), (b) it discharges to the already-built `hneRel_isBisim`, and (c) it recovers the two instances honestly.

**Ambient theory.** `spec/README.md` §2: `PkObj`/`PkMap`; `IsBisim`; `SHNE` (strong hereditary non-emptiness); `hneRel`/`hneRel_isBisim` (transcribed from Series 06 `ws2.lean`); `bisim_eq` on `νPk`.

## Candidates

### C1 — Existential-bisimulation form (the lead; transcribes Series 06)
...
- **Failure mode:** none structural (the proof is already built in Series 06); the only risk is that "∃ a bisimulation" is *too weak* to be useful downstream if a workstream needs the *greatest* bisimulation or genuine bisimilarity. Checked against consumers below.

**Paper triage.** Decidable-on-paper and already proved: `hneRel_isBisim` exists verbatim in Series 06 `ws2.lean`. C1 is that lemma with the two hne states threaded in. Every downstream consumer (WS2's collapse, WS3's history bridge) needs only "∃ bisimulation relating them", not the greatest one, because behavioral identity (WS2) contains *every* bisimulation in equality. **Winner.**
...
/-- **Dynamic instance.** The productive thread of the process is unique — Series 06's
...
**D1 — the engine.** `ws1_atomless_bisim`. *Strategy:* the term above; `hneRel_isBisim` is transcribed. *Paper-decidable:* yes — it is a proved Series 06 lemma re-exported with the hne hypotheses threaded.
...
`series-07/formal/Series07/ws1.lean`: transcribed carrier + `hneRel`/`hneRel_isBisim`; `ws1_atomless_bisim` (D1), `ws1_recovers_static` (D2), `ws1_recovers_dynamic` (D3). Axiom check: `#print axioms ws1_atomless_bisim` on the standard three (it is `hneRel_isBisim`, already axiom-clean in Series 06).
