# Series 09 — Axiom check log

**The committed record of `#print axioms` over every Series 09 headline, and — the single most important
...
Run: `lake build Series09.AxiomCheck` (see `formal/Series09/AxiomCheck.lean`). Toolchain
...
(`lake build`: Series07 + Series08 + Series09) builds clean, sorry-free, warning-free.

**Re-verified as a captured run (2026-07-11, series-review-1 F-10, second review-address pass).** The
axiom lines below were captured from an actual `lake build Series09.AxiomCheck` invocation (stdout
...
```
theorem Series09.WS1.ws1_no_self_total_hold.{u} : ∀ {κ : Cardinal.{u}} {X : Type u} (dest : X → PkObj κ X)
...
Series 08 coincidence is NOT re-hit.** This is the whole repair of Series 08, confirmed at the proof-term
...
```
'Series09.WS1.ws1_no_self_total_hold' depends on axioms: [propext, Classical.choice, Quot.sound]
```
...
`ws7_verdict = Series09Verdict.selfReferenceEstablished` by `rfl` on the discharged `Audit` (whose
