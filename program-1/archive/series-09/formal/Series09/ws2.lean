/-
`series-09/formal/Series09/ws2.lean`

WS2 — **The residue breaks the collapse from one position.** Series 09, the payoff.
...
premise — the exact repair of Series 08's circularity (`x↾(x,y) ≠ y↾(y,x)` needed `x ≠ y`). Series 08's
distributed perspective recovers as a special case (mutual residues), not the source.

Depends on WS1 (the diagonal spine and its independence). Design doc: `series-09/spec/ws2-design.md`.
...
second position in the premise — the repair of Series 08's `x↾(x,y) ≠ y↾(y,x)` needing `x ≠ y`. A hold
...
Series 08 coincidence) BECAUSE the spine is the independent diagonal (WS1's `ws1_no_self_total_hold`,
...
/-- **D5 — distributed perspective as a special case, scoped honestly.** Series 08's `labelLoop` pair
