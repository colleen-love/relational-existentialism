/-
`series-04/formal/ws3.lean`

WS3 — **Plurality without atoms.** Series 04.
...
`q : Q` (the face, as functor data). This is the exact analogue of the Series 03
...
open Cardinal QPF Functor Series04.WS1

namespace Series04.WS3
...
carrier. The direct analogue of Series 03 `ws14_loop_ne` (distinctness by weight),
here by face. -/
...
theorem ws3_same_succ_diff_face (hinf : ℵ₀ ≤ κ) {q₁ q₂ : Q} (hne : q₁ ≠ q₂) :
    ...
    (∀ a b : (νPk κ).X, Series04.WS2.HereditarilyNonempty a →
        Series04.WS2.HereditarilyNonempty b → a = b)
    -- plural half: with faces, same-target loops split by face
  ∧ (loopState q₁ hinf ≠ loopState q₂ hinf) :=
  ⟨fun a b ha hb => Series04.WS2.ws2_collapse hinf a b ha hb, ws3_loopface_ne hinf hne⟩
...
already excluded. This is *stronger* than Series 03 `ws14`, which genuinely leaked at the
