# Relational Existentialism — Series 06: Charter Status
...
*Why this file exists: the charter is the stable statement of intent, edited only to fix errors in the design itself. This file absorbs everything that changes as work proceeds. When you want to know "what does Series 06 claim," read the charter. When you want to know "where is Series 06," read this. (Series 04 introduced this split after Series 03 let REV-A…REV-H amendments pollute its charter inline; Series 05 and 06 keep the discipline.)*
...
- **Design status** (Series 06 batches like Series 05): each workstream first reaches **Design committed** (a stable contract, no Lean) before any proof status. See `protocol.md` §2.
...
| **This status file** | v1 — designs committed (Phase B) and the full build landed (Phase C). All seven `wsN.lean` + `Series06.lean` + `AxiomCheck.lean` compile, **sorry-free and axiom-clean** (standard three only; `spec/axiom-check-log.md`). Closure gate passes (self-contained). |
| **Design docs** | **Committed.** `spec/ws1…ws7-design.md` + `spec/README.md`. |
| **Formalization** | **Built.** `series-06/formal/ws1.lean … ws7.lean`, `Series06.lean`, `AxiomCheck.lean`. Self-contained (no `series-04/`, `series-05/`, `archive/` imports; gate OK). Pinned Lean 4 v4.15.0 / Mathlib v4.15.0. |
...
*Series 06 batches review at the whole-series level (see `protocol.md` §2–§3): a review session reads all built code against all design contracts and charter criteria, blind to motivating prose, and writes `series-review.md`; a code session addresses it; repeat.*
