# Relational Existentialism — Series 07: Charter Status
...
*Why this file exists: the charter is the stable statement of intent, edited only to fix errors in the design itself. This file absorbs everything that changes as work proceeds. When you want to know "what does Series 07 claim," read the charter. When you want to know "where is Series 07," read this. (The split is inherited from Series 04/05/06.)*
...
| **This status file** | v1 — designs committed (Phase B) and the full build landed (Phase C). All seven `Series07/wsN.lean` + `Series07.lean` + `Series07/AxiomCheck.lean` compile, **sorry-free and axiom-clean** (standard three only; `spec/axiom-check-log.md`). Closure gate passes (self-contained; nothing imported across series). |
| **Design docs** | **Committed.** `spec/ws1…ws7-design.md` + `spec/readme.md` (moved into `spec/`). |
| **Formalization** | **Built.** `series-07/formal/Series07/ws1.lean … ws7.lean`, `Series07.lean`, `Series07/AxiomCheck.lean`. Self-contained. Pinned Lean 4 v4.15.0 / Mathlib v4.15.0. |
...
| **Prior art (transcribed, not imported)** | The `P_κ`/bisimulation machinery + the process (`Approx`/`Proc`/`allNonempty_unique`/`ws1_productive_unique`) + `hneRel`/`hneRel_isBisim`/`Static`/`twoLoop`, all transcribed into `Series07/ws1.lean`, re-namespaced. The engine (WS1) is their generalization. |
...
7. **Machine-checked axiom pass** — all. **CLOSED.** `Series07/AxiomCheck.lean` built; `spec/axiom-check-log.md` records every headline theorem sorry-free on the standard three.

**Residual opens after the build:** #3 (trichotomy exhaustiveness across *any construction* — the un-rangeable quantifier, genuinely Partial) and the metric `CauchySeq` packaging of the loophole family (WS5, build-owed). Neither is a dependency of the Impossibility; both are pre-registered honest Partials. **The program's recurring lesson, confirmed and turned into the deliverable:** the sharpest result is a proved impossibility. Series 07 makes the impossibility the whole point and it lands — non-circular (the risk that mattered), with the trichotomy's completeness the honest ceiling exactly where the charter placed it.
...
- **Phase C (build-all)** complete: all seven `Series07/wsN.lean` + `Series07.lean` + `Series07/AxiomCheck.lean` compile, sorry-free and axiom-clean; closure gate passes.
...
- **Phase E (address), pass 1** complete (2026-07-10). Every finding addressed under the executor discipline (prove-more-or-relabel, never lower the bar). Summary:
  ...
  - **C1/C2 (framing) — acknowledged.** Series 07's delta over Series 06 is honestly a reframing + the general bisimilarity lemma; the "live importForced arm" is gated on the open exhaustiveness flag and not implied to be near.
  Full build re-verified sorry-free, axiom-clean; gate passes.
- **Phase D (blind review), interim review — CLEAN but DISCOUNTED** (against commit `cdf26d3`; its write-up was later overwritten in `spec/project-review-2.md` by the alignment pass below): four reviewers (two fix-validators, two fresh from-scratch, one tasked solely with building a counterexample). **No SERIOUS finding at the term level.** All four pass-1 SERIOUS items independently confirmed genuinely fixed at the term level (S1 fully; S3/S4 fully; S2 re-scoped to an honest Partial); the fresh from-scratch attack could not build a counterexample to the Import Theorem and confirmed *why* (the genuine unlabelled-powerset collapse, not a rigged `IsBisim`); AxiomCheck coverage confirmed to omit none of the load-bearing headlines. **This review is discounted** (not counted as pass 2): clean at the term level, it nonetheless missed the substantial alignment issue that the next pass caught. Its C1–C4 items were still addressed (below).
- **Phase E (address), interim review** complete (2026-07-10). Only cosmetic/terminal items (C1–C4) — all relabels/removals, **no mathematics changed**, verdict unchanged:
  ...
  - **C2 (interim review) — `HistoryDiff → LeafyThreadDiff`** (and `ws3_history_kind_* → ws3_leafy_thread_*`): honestly a leafy-thread (leaf) difference, not the design's same-limit haecceity kind. Added `ws3_no_same_limit_haecceity` recording the genuine haecceity witness as **structurally absent** (any two productive threads are equal) — a named open, the sharpest statement of what the program has circled and arguably the seed of a Series 08.
...
- **Phase D (blind review), pass 2 — ALIGNMENT-to-charter** (`spec/project-review-2.md`, the counted second pass — the interim clean review above is discounted): a single reviewer graded whether the proved theorems are theorems about the *right object* — the one dimension a term-level audit structurally cannot catch. It disputes no proof and no axiom; it found the build **conflated "labelled" (a `Q` in the signature) with "imported" (the charter's §4.1 "coordinate NOT carried by the relating")**, and that the conflation reaches WS4's catalogue and WS7's audit. This is a genuine, correct finding — the substantial issue the interim review missed, which is why that review is discounted and this is pass 2.
- **Phase E (address), pass 2** complete (2026-07-10). Addressed charter-strength-first (prove the real predicate; re-classify honestly; never a relabelled charter target):
  ...
  - **S2/R1/R2 (the S4 restriction is not an import) — FIXED (re-classified honestly).** The Series 04 face `x↾(x,y)` is a restriction of the relatum, hence recoverable, hence by `ws4_recoverable_not_import` **NOT an import** — it is a leaf / **faced boundary** (a descent boundary that also carries a quality). Series 04 reached plurality only by ESCALATING the restriction into a FREE label (`loopState q`), which IS a genuine import; the two objects are different, keeping one name. `ws4_program_explained` and its realization note are corrected: the program is explained *for free-label imports*; the endogenous S4 (and possibly S5) cases are recoverable, reclassified. WS3 records the **faced boundary as the candidate FOURTH kind** (`ws3.lean` note) — reducing it to leaf-or-import is the load-bearing open, NOT assumed; it is the interim review's C2 haecceity seen from the label side. The faithfulness gap (minimal free-label witness ≠ the S4 restriction) is now disclosed, not just the coverage gap.
  ...
  **Verdict scope, made precise (the honest outcome pass 2 asks for):** the **plain collapse** — the Import Theorem `ws2_import_theorem` and its engine — stays `payoffsEstablished`, untouched and attack-tested. The **free-label import mechanism** is now witnessed at charter strength (semantic). The **prior-series catalogue** (WS4) is honestly **re-classified / Partial**: only free-label imports are certified as imports; the Series 04 restriction is a leaf/faced-boundary, and its plurality is a free-label escalation. The **faced boundary** is a named candidate fourth kind (open). No charter target was relabelled; the charter's §4.1 semantic definition was the fixed bar the build is now aligned to.
  Full build re-verified sorry-free, axiom-clean (38 headline theorems, standard three); gate passes.
- **Phase E (address), pass 2 — follow-up: the recoverable-label collapse mechanized** (2026-07-10). A review of pass 2's WS4 exposed that the recoverable-not-import direction (`ws4_recoverable_not_import`) was near-definitional and conditional, with `Recoverable` uninhabited and the "S4 restriction collapses" claim left as prose. Both gaps closed, charter-strength-first, no charter target relabelled:
  - **Gap 1 (abstract collapse) — MECHANIZED.** `ws4_recoverable_atomless_collapses`: a recoverable label + behavioral identity + atomlessness of the plain relating ⇒ `Subsingleton` (via `ws2_import_theorem_static` on `plainOf`). A recoverable label imports nothing and cannot rescue atomless plurality. `ws4_recoverable_plurality_requires_drop` is the contrapositive. Not the un-rangeable quantifier — a clean general theorem, previously simply absent.
  - **Gap 2 (the S4 witness) — MECHANIZED on minimal witnesses.** `Recoverable` is now inhabited: `facedLoop` (a restriction/constant label, the shadow of `x↾(x,y)` on collapse-equal states) is proved `Recoverable` (`ws4_facedLoop_recoverable`) and atomless, with its plurality a DROP of behavioral identity (label-bisimilar states). `ws4_labelLoop_not_recoverable` proves the free label is genuinely NOT recoverable — the import. `ws4_restriction_collapses_escalation_imports` packages the contrast. The literal `νLk` carrier stays prior art; the only remaining prose is the modeling identification of the actual face with the recoverable class.
  - **Honest residual.** What is now a theorem: the restriction collapses (recoverable ⇒ import-free ⇒ subsingleton-under-behav-id-atomless) and the free-label escalation imports. What stays prose: that the specific `νLk` face lands in the recoverable class (a modeling claim, would need the `νLk` carrier transcribed). The plain-collapse verdict is unchanged.
  Full build re-verified sorry-free, axiom-clean (42 headline theorems, standard three); gate passes.
- **Phase E (address), pass 2 — follow-up: subset-of-a-restriction, tightness of the `atomless` hypothesis** (2026-07-10). Probe: a distinction whose material is entirely endogenous (a *subset of a restriction*) but whose SELECTION may be free per state — does the collapse eat it? Resolved by the engine `ws1_atomless_bisim` (atomlessness makes all states plainOf-bisimilar), both directions mechanized:
  - **Case A (selection fixed by the relating) — collapses.** `ws4_atomless_recoverable_all_bisimL`: on an atomless recoverable carrier every pair is label-bisimilar; a relating-determined subset exposes nothing new. Over atomless carriers this forces the selection globally constant (witnessed by `facedLoop`), and behavioral identity collapses it.
  - **Case B (selection free per state) — SURVIVES, but as an import; not a counterexample.** `ws4_atomless_label_distinction_imports`: on an atomless carrier any label distinction that is not label-bisimilar is necessarily non-recoverable (an import), *regardless of what the label is made of*. Concrete witness `subsetLoop` (labels are literal subsets of the state space, `subsetLabel i = {i}`): atomless, plainOf-bisimilar, survives at the label level, and NOT recoverable (`ws4_subset_selection_survives_as_import`). The atom is in the free CHOOSING, not the endogenous material.
  - **Verdict.** No counterexample to `ws2_import_theorem` (that theorem is plain-only; Case B is labelled). The `atomless` hypothesis is **tight**: atomlessness is exactly what forces every non-importing selection to be globally constant, so per-state variation *requires* an import. The "subset of a restriction with no atom" dissolves — freedom over endogenous material is itself the atom. Charter untouched; plain-collapse and recoverable-collapse verdicts unchanged.
  Full build re-verified sorry-free, axiom-clean (45 headline theorems, standard three); gate passes.
- **Phase E (address), pass 2 — capstone: the atom-or-will fork** (2026-07-10). The subset/attention thread was pushed to its limit: can a *dynamic* attention process make two atomless histories unequal without an atom? Resolved by `ws1_productive_unique` (atomless histories are EQUAL, not merely bisimilar — both are Ω):
  - **`att_cannot_distinguish_atomless_histories`** (mechanized). Any `att : Proc κ → Q` gives equal atomless histories equal values — a function cannot separate equal inputs. So any separator of atomless histories must read an argument exogenous to the relating.
  - **The honest disjunction (the result the impossibility deliberately does not close).** That exogenous argument is, to the formalism, a coordinate not carried by the relating — the §4.1 footprint of an import. But the same footprint is left by two philosophically opposite things: a **given** (a pre-existing atom) and a **choice** (a distinction freely originated / free will). Type theory has no predicate for "freely chosen rather than given," so the theorem forces the disjunction (atom OR will) and is silent on the disjunct. The undecided disjunct — choice — is the fourth kind the leaf/import dichotomy could never name, because to the relating a free choice and an imported atom are the same footprint.
  - **The arc.** Read as Parmenides generalized: a groundless, atomless, faithfully-relational world that is DETERMINED is the One (`ws1_productive_unique`; Series 06's collapse is the world without will). Plurality requires a distinguisher not carried by the relating — an atom or a will. This does not touch the theorem or the verdict; it names, precisely, the philosophical fork the machine-checked impossibility leaves open. The §4.2 "drop 2 re-imports" prose is correspondingly too strong: bare distinctness (`twoLoop`, plain, import-free) and free choice are drop-2 phenomena that carry no §4.1 atom.
  Full build re-verified sorry-free, axiom-clean (46 headline theorems, standard three); gate passes.
- **Phase D (blind review), pass 3 — adversarial series review** (`spec/project-review-3.md`): a whole-series adversarial pass (strip test, laundering, coincidence/independence, trivialization verdict). **No SERIOUS finding.** The engine is not painted on, the flagship does not launder, every hypothesis is load-bearing, the escapes are refuted as theorems; verdict `payoffsEstablished` confirmed correct; **not trivial**. Three correctly-labelled Partials (R1–R3) and cosmetics (C1–C5). (The reviewer could not run the toolchain in their environment; the axiom-check is machine-checked here at this commit.)
- **Phase E (address), pass 3** complete (2026-07-10). Prove-more / complete-the-check, no bar lowered:
  - **R1 (drop-1 witness was only first-level nonempty) — FIXED (prove more).** `ws4_label_drop_atomless`: the labelled witness's plain relating is HEREDITARILY atomless (`∀ i, SHNE (plainOf (labelLoop hinf)) i`, via `plainOf_labelLoop_reaches` mirroring `twoLoop_HNE`), plus behaviorally-identified, plural, and survives-quotient. The drop-1 row of the catalogue now meets ingredient (3) at the theorem's own strength, not a weakening.
  - **C5 (AxiomCheck omitted the verdict `def`s) — FIXED.** `#print axioms` now also covers `ws7_verdict`, `ws5_loophole_adjudication`, `ws6_universal` (the three reported verdict constants). 50 headline lines.
  - **R2 / R3 — kept as the honest Partials the review endorses** (do not promote): WS4 "program explained" is proved for the free-label mechanism and reclassified/prose/absent for S4/S5/S3 (the S3 weight carries *no* Series-07 witness; the generic import witness is a free label); WS3 is a `ws3_dichotomy`, not the trichotomy, with exhaustiveness the acknowledged un-rangeable open. The summaries and this ledger avoid asserting the full target.
  - **C1–C4 — acknowledged as disclosed/acceptable** (the `Iff.rfl` NC1 conjunct is a declared definitional alias, pulled out of the load-bearing anchors; the atom-or-will capstone is a true lemma plus an interpretation, never cited as a proved disjunction).
  Full build re-verified sorry-free, axiom-clean (50 headline lines — 47 theorems + 3 verdict `def`s, standard three); gate passes. NB: PR #76 merged this work into `main`; this pass 3 is fresh follow-up on the branch restarted from `main`.
...
- **REV-0 (initial):** charter created; no changes yet. Series 07 seeded from Series 06's honest finding (the process collapse, `ws1_productive_unique`) per the program's inter-series discipline (protocol §6): the prior series' hardest pushback becomes the next series' question. Series 06 is **not** rewritten — it stays the honest record of the dynamism attempt; Series 07 is the standalone impossibility that generalizes it.
