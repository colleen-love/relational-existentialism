# Relational Existentialism — Series 07: Technical Status Summary
...
- The full prior carriers (`νLk`, `Winf`, the weight algebra) are prior art; the drop *mechanisms* are mechanized on minimal witnesses. The S3 weight carries *no* Series-07 witness (the generic import witness is a free label, not a weight); the S5 index is asserted "reuses the label mechanism" in prose (R2).
