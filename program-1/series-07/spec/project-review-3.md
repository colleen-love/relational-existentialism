# Series 07 — Adversarial Series Review

*Reviewer: adversarial pass across the whole of Series 07 at once. Object of review: does the Lean
...
Method: source audit of all seven workstreams plus `Series07.lean` and `AxiomCheck.lean`, the strip
...
cache, so `lake build` / `lake env lean Series07/AxiomCheck.lean` could not run. **Everything below
...
Series 04 face `x↾(x,y)` is **reclassified as recoverable, hence not an import** (a leaf / faced
boundary), and Series 04's plurality is recast as a free-label *escalation*. Series 03 weights and the
Series 05 `Winf` index are **not** mechanized at all (S3 explicitly Partial; the index is asserted
...
fixed bar, and the build aligned to it). The residual owed is (i) a Lean witness that the Series 05
...
that Series 03 weights carry *no* Series-07 witness (currently "covered generically," which overstates
...
- **The semantic import test — `ws4_free_label_is_import`, `ws4_recoverable_not_import`,
  ...
  reclassification of the Series 04 face as recoverable-hence-not-import is mechanized on minimal
...
v4.15.0 / Mathlib v4.15.0 toolchain should run `lake env lean series-07/formal/Series07/AxiomCheck.lean`
...
The spine of Series 07 is real and clean. The engine (`ws1_atomless_bisim`), the Import Theorem
