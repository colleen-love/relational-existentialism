...
The underlying relating is Series 7's **plain `P_κ`-coalgebra**, transcribed verbatim:
...
**This is the home. No workstream may pick a different one.** The terminal/QPF `νPk` is *not* transcribed (Series 7 already showed the abstract `BehaviorallyIdentified` form subsumes it); Series 8 works over an arbitrary `dest : X → PkObj κ X`.

### 2.2 The hold / restriction primitive (WS1 fixes it; holding-first, charter §3)

A **hold** is a directed edge, read as *the source restricting the relation toward the target*. Following Series 4's `face` (`x↾(x,y)` = the part of `x` routed through its edge to `y`):
...
**Perspective** is the *directedness*: the hold `(x,y)` (x's face toward y) is a different object from `(y,x)` (y's face toward x). The two are carried by the Series 7 **labelled functor** `LkObj κ Q X = PkObj κ (Q × X)`, with `Q` recording the source-direction; `plainOf` forgets it. A face is **recoverable** (`Recoverable dest`) iff the direction adds no constraint the plain relating does not already impose — the god's-eye/symmetric case — and **free** otherwise. This is the Series 4/WS4 semantic import test, transcribed, and it is the hinge of the whole series (§2.3).
...
**This is the order. WS3 and WS4 both consume `prec`; neither may redefine it, and neither may substitute an external `ℕ`-index.** The imported-index branch (`≺` := a stamped stage counter, the Series 5 trap) is designed in as a *refuted* failure mode (`ws3_imported_index_refuted`), never a fallback. **Breadth** is measured *separately* from the map, so conservation stays testable (§2.5).
...
- **The face primitive (Series 4 `ws1`):** `face` / `RoutedThrough` / `ws1_face_forced` — re-expressed as `Hold` / `afford` / `ws1_hold_forced` on this carrier (§2.2).
