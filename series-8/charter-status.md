...
| Item | Status |
|---|---|
| **Headline target** | The no-god's-eye theorem: no relationally-identified node holds all perspectival restrictions. **Impossibility proved** (`ws1_no_gods_eye` / `ws1_no_gods_eye_node`), pending blind review. |
| **The three consequences** | Plurality (`ws2_perspective_breaks_merge`, monism-broken), forced dynamics (`ws3_dynamics_forced`, endogenous order), layering-as-narrowing (`ws4_depth_is_narrowing`). **Discharged on the mechanized core**, pending review. |
...
| **Verdict (WS7)** | `perspectiveEstablished` (`ws7_verdict`, a function of the discharged `Audit`). Circularity guards discharged: no conservation-by-fiat (`ws7_no_conservation_by_fiat`), no freeness-defined-in (`ws7_freeness_not_defined_in`). Pending blind review. |
...
**Status: Impossibility proved (Discharged) — pending blind review.** Built in `formal/Series8/ws1.lean`.
...
**Status: Discharged (monism-broken) — pending blind review.** Depends on WS1 (freeness). Built in `ws2.lean`.
...
**Status: Discharged — pending blind review.** (NL)+(NF) + forced dynamics + endogenous order all built in `ws3.lean`. **Design fix logged** (see closed log): `ws3_imported_index_refuted` signature corrected from the unrealizable `h ≠ h'` to the self-loop step `ReReStep h h`.
...
**Status: Discharged on the mechanized core — pending blind review.** Antitone-`afford` + reach-as-trace built in `ws4.lean`; universal form remains the pre-registered Partial (WS6).
...
**Status: Discharged — pending blind review.** Floor (`ws6_provable_core`) + retraction ledger (`ws6_conservation_retracted`) built; universal theses flagged, not claimed as theorems. Built in `ws6.lean`.
...
**Status: Discharged — pending blind review.** Mechanized `Audit` + typed verdict `perspectiveEstablished`; both circularity guards discharged. Built in `ws7.lean`.
...
|---|---|---|---|
| No god's-eye node (spine) | WS1 | **Impossibility proved** (`ws1_no_gods_eye`) | Yes — collapsed by the engine (`ws4_recoverable_atomless_collapses`), not excluded by fiat; symmetric hold proved `Recoverable` (`ws1_symmetric_hold_recoverable`). Strip flagged (see below). |
...
*(empty — Phase C build complete; blind whole-series review (Phase D) not yet run)*
