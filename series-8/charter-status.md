...
| Item | Status |
|---|---|
| **Headline target** | The no-god's-eye theorem: no relationally-identified node holds all perspectival restrictions. **Not started.** |
| **The three consequences** | Plurality (WS2), forced dynamics (WS3), layering-as-narrowing (WS4). **Not started.** |
...
**Status: Not started.** Depends on WS1.
...
**Status: Not started. THIS WORKSTREAM OWNS THE CENTRAL OPEN AND IS FORBIDDEN FROM ASSUMING ITS ANSWER.**
...
|---|---|---|---|
| No god's-eye node (spine) | WS1 | Not started | TBD (Spinozist-retreat is the test) |
| Perspective breaks collapse | WS2 | Not started | TBD (freeness must not be defined-in) |
| Re-restriction: no leaf (NL) | WS3 | Not started |, |
| Re-restriction: not-a-function (NF) | WS3 | Not started |, |
| Dynamics forced by finitude | WS3 | Not started | TBD (order must be endogenous) |
| Layering = narrowing | WS4 | Not started |, |
| Conservation of breadth | WS5 | **Open by design** | TBD (kill condition is the guard) |
...
*(empty, series not yet begun)*

## Series-review log

*(empty, no build to review yet)*
