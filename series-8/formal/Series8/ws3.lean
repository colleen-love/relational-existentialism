/-
`series-8/formal/Series8/ws3.lean`

WS3 — **Re-restriction and forced dynamics.** Series 8, the engine of Part Two.

Genuinely new Lean (the carrier is transcribed via `Series8.ws1`; the re-restriction map, the
order, and forced dynamics are Series 8 content). Owns the re-restriction map, its two cheap
...
Design doc: `series-8/spec/ws3-design.md`. DESIGN FIX (recorded in `charter-status.md`):
