...
- **"One finitude" is a conjunction, not a derivation (review R1).** `ws7_payoffs_hold` (renamed from `ws7_one_finitude`) conjoins six independently-proved payoffs; it does *not* take `FinitudeOfFacing` as hypothesis and derive them. Only one payoff genuinely derives from finitude (`ws7_incompleteness_off_from_finitude`, the off-diagonal properness = its first clause). Distinctness is mechanized for two pairs only (`ws7_deductions_dont_collapse`, `ws7_plurality_vs_collapse_distinct`). The typed verdict is downgraded from `oneFinitude` to **`payoffsEstablished`**.
