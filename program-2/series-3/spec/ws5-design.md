...
alternatives `convergenceDecided` / `disconnected` / `partial'` reachable. Audit clauses (a)-(e) bundled.
...
/-- `shapeDrawn` iff `Converges₂` is typed and two-sided free (WS1), forced over the in-sight class AND its
    ...
    `disconnected` (WS1 fails), `partial'` (WS2 or WS3 degenerate), `convergenceDecided` (the in-sight forcing
...
/-- **Falsifiability.** The verdict DISCRIMINATES: flipping a flag computes a different outcome. The
    `convergenceDecided` arm (in-sight forcing extends to the full class) is a reachable input — the
    pre-registered stronger outcome, never hand-set on this witness. -/
theorem ws5_verdict_discriminates :
    verdict true true true false = Outcome.convergenceDecided
...
`ws5_flags_justified := ⟨ws1_no_orientation_evaluated hinf, ws2_converges_decided_in_sight hinf,
...
/-- (b) THE DIRECTION IS NEVER DECIDED. No theorem, definition, or discharged obligation states that `slf` and
    ...
    `shapeDrawn` and `convergenceDecided`) and fills neither. A NAMES/absence property about the build, carried
...
theorem ws5_audit_fork_genuine (hinf : ℵ₀ ≤ κ) :
    (fork bundle : ws4_two_zone_convergence hinf) ∧ (proper : ws4_insight_proper hinf)
...
WS3 dissent an import, WS4 both zones witnessed), so `verdict = shapeDrawn`. `convergenceDecided` is reached by
