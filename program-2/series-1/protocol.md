...
Read the canonical templates first: `program-1/series-13/spec/*` for the design-doc format (candidates, triage table, typed signatures, strip annotation) and `program-1/series-13/formal/Series13/*` for the Lean house style. **Also read `program-2/series-0/` in full** (`charter.md`, `charter-status.md`, `formal/P2S0/*`) — it is the ground S1 imports, and its actual API (`attends`, `reifyU`/`finTowerN`, `ws1_first_other`, `rankLift`, `knowLiftD`, `impLift`) is what WS1–WS4 build on. For each of WS1–WS5, write `spec/wsNN-design.md`:
...
Build `formal/` to the Phase B (post-repair) signatures, one module per workstream plus `AxiomCheck`. **Build on the Series 2.0 ground** (`program-2/series-0`, namespace `P2S0`): `import P2S0` and use its attention carrier (relating is finite attending) as the base. The layered import chain is `P1 → S0 → S1` (program charter §2): S0 imports P1, so `P1.Core.*` names are reachable TRANSITIVELY through S0 — do NOT `import P1` directly, so all Program 1 machinery stays mediated by S0's finite-attention ground. Importing any other series' tree is forbidden; the gate enforces it. **S0 has landed** (GROUND-ESTABLISHED, resting on `ws1_first_other`), so the precondition is met: build on S0's OWN API (`attends`, `outDest`/`symDest`, `FinReify`/`reifyU`/`finReifyStep`/`finTowerN`, `ws1_first_other`, `rankLift`, `knowLiftD`, `impLift`) — NOT P1's `PkObj`/`FiniteAttention`. The tick generalizes `ws1_first_other` (the self-loop reified is the base case), and per charter §3 the tick is where S0's two labels (`rankLift` rank, `knowLiftD` direction) merge into one act — a WS1 design opportunity to design for, not inherit split. Heed the foundation's two guardrails (`P1.lean` banner): do NOT reuse or reconstruct the Series 12 WS3/WS4 compass/convergence machinery (PR1-S1, not present in the foundation), and use `P1.Reader.ws2_attention_makes_real` / `RealFor` (reader-load-bearing) as the template for any plurality/perspective payoff, never `Many` (PR1-S2).
...
- In `scripts/gate.sh`, add: `check program-2/series-1 "^import (P2S0|P2S1)(\.[A-Za-z0-9_]+)*$"` (the series imports its predecessor S0 and its own `P2S1.*` roots, plus Mathlib; P1 is reached transitively through S0, not imported directly; any other series' tree is forbidden).
