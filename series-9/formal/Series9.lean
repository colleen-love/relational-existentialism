/-
`series-9/formal/Series9.lean`

Series 9 — **Self-reference is the engine.** The aggregator root.

The program's return to its founding thesis, and the response to Series 8's honest Partial: Series 8's
perspective spine COINCIDED with relational identity (`ws1_symmetric_states_bisimilar`), leaving open
whether no-god's-eye is separable from relational identity. Series 9 changes the engine from perspective
to self-reference.
...
by `ws1_diagonal_not_bisim`. This is the repair Series 8 could not make: no-self-total-hold is the
...
`ws2_residue_distinct`), repairing Series 8's circularity; forced dynamics — the re-diagonalization map,
...
This file is SELF-CONTAINED: every Series 8/7/4 lemma is transcribed into `Series9/wsNN.lean` and
re-namespaced `Series9.WSn` — nothing is imported from `series-8/`, `series-7/`, `series-4/`, or
`archive/`. `Series9.AxiomCheck` runs `#print axioms` over every headline. Sorry-free; axiom-clean
