/-
`series-09/formal/Series09/ws7.lean`

WS7 — **The anti-circularity audit.** Series 09, owns the verdict.

The audit against the four Series-09 circularity risks — the-diagonal-is-really-a-bisimulation (the
...
`Series09Verdict` that is a function of it, so it cannot be hand-set. The coincidence rule is PROMOTED to
a first-class spine check (`ws7_coincidence_check`).

Runs last; consumes WS1–WS6. Design doc: `series-09/spec/ws7-design.md`.
...
open Series09.WS1 Series09.WS2 Series09.WS3 Series09.WS4 Series09.WS5 Cardinal
...
inductive Series09Verdict | selfReferenceEstablished | coincident | monismStands | Circular
...
def ws7_verdict : Series09Verdict := verdict (ws7_audit (κ := κ))

theorem ws7_verdict_eq : ws7_verdict (κ := κ) = Series09Verdict.selfReferenceEstablished := rfl
...
theorem ws7_audited_not_coincident (cert : Audit κ) :
    verdict cert ≠ Series09Verdict.coincident := by
  show Series09Verdict.selfReferenceEstablished ≠ Series09Verdict.coincident; decide

theorem ws7_audited_not_monism (cert : Audit κ) :
    verdict cert ≠ Series09Verdict.monismStands := by
  show Series09Verdict.selfReferenceEstablished ≠ Series09Verdict.monismStands; decide

theorem ws7_audited_not_circular (cert : Audit κ) :
    verdict cert ≠ Series09Verdict.Circular := by
  show Series09Verdict.selfReferenceEstablished ≠ Series09Verdict.Circular; decide

/-- **D4 — the coincidence check (the flagship).** The spine is diagonal-not-bisimulation, orthogonal to
relational identity, contrasted IN THE SAME STATEMENT with the Series 08 coincidence witness
`ws1_symmetric_states_bisimilar` (which needs `Symmetric` and PRODUCES a bisimulation, whereas the spine
DENIES a fixed point). The certificate that Series 09 escaped Series 08's wall. -/
