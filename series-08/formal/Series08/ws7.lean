/-
`series-08/formal/Series08/ws7.lean`

WS7 — **The anti-circularity audit.** Series 08, owns the verdict.

Audits the two Series-08 circularity risks — conservation-by-fiat and
perspective-recoverable-by-definition — aggregated into a mechanized `Audit` certificate whose every
field is a theorem, and a typed `Series08Verdict` that is a function of it (transcribing Series 07's
`Audit`/`verdict` pattern), so it cannot be hand-set. Runs last; consumes WS1–WS6.

Design doc: `series-08/spec/ws7-design.md`.
...
open Series08.WS1 Series08.WS2 Series08.WS3 Series08.WS5 Cardinal

variable {κ : Cardinal.{u}}

/-- Series 08's three outcomes. -/
...
theorem ws7_verdict_eq (hinf : ℵ₀ ≤ κ) :
    ws7_verdict hinf = Series08Verdict.perspectiveEstablished := rfl
...
theorem ws7_audited_not_monism {hinf : ℵ₀ ≤ κ} (cert : Audit κ hinf) (b : Bool) :
    verdict cert b ≠ Series08Verdict.monismStands := by
  show Series08Verdict.perspectiveEstablished ≠ Series08Verdict.monismStands; decide

theorem ws7_audited_not_circular {hinf : ℵ₀ ≤ κ} (cert : Audit κ hinf) (b : Bool) :
    verdict cert b ≠ Series08Verdict.Circular := by
  show Series08Verdict.perspectiveEstablished ≠ Series08Verdict.Circular; decide
