/-
`series-08/formal/Series08/ws2.lean`

WS2 — **Perspective breaks the collapse.** Series 08, the payoff.

Two finite holds of a shared relation, taken from opposite sides, fail "relate alike" so the
Series 07 merge does not apply — plurality with no import (vs. weight) and no leaf (vs.
limit-atomlessness), provided the asymmetry is FREE, which WS1 secures. The witnesses are the
transcribed free-label machinery (`Series08.ws1`); WS2 re-reads them as "distinct perspectives
do not merge" and delivers the freeness verdict.

Design doc: `series-08/spec/ws2-design.md`. Depends on WS1 (the global freeness the payoff leans on
...
/-- **D1 — perspective breaks the merge.** Plain-bisimilar (Series 07 would merge them) yet the
