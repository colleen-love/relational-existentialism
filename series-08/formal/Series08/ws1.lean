/-
`series-08/formal/Series08/ws1.lean`

WS1 — **The perspective primitive and the no-god's-eye theorem.** Series 08, the blocking spine.

This file is SELF-CONTAINED: it transcribes (re-namespaced `Series08.WS1`) the plain `P_κ` /
bisimulation machinery, the labelled/face machinery, and the collapse from Series 07 —
nothing is imported from `series-07/`, `series-04/`, or `archive/`. It fixes the carrier and the
...
Design doc: `series-08/spec/ws1-design.md`; shared objects `series-08/spec/README.md` §2.
...
/-! ## The κ-bounded powerset functor `P_κ` (transcribed from Series 07 `ws1`) -/
...
/-! ## The hold primitive `x↾(x,y)` (Series 08, holding-first — charter §3) -/
...
carrier, it does not annotate it. (Series 04 `ws1_face_forced`, transcribed.) -/
...
making no-god's-eye a fact separate from the Series 07 collapse. See `charter-status.md` (pass 3) and
`ws1-design.md`. -/
