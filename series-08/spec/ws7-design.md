...
**Design doc. Series 08, owns the verdict. Owns: the audit against the two Series-08 circularity risks — conservation-by-fiat and perspective-recoverable-by-definition — aggregated into a mechanized `Audit` certificate whose every field is a theorem, and a typed `Series08Verdict` that is a function of it, so it cannot be hand-set.**

*Series 08 is standalone; names transcribed into `series-08/formal/Series08/ws7.lean`, re-namespaced `Series08.WS7` (see `spec/README.md` §6). WS7 **runs last** (protocol §4): it audits WS1–WS6 and cannot report until they have. The `ProgramVerdict`/`Audit`/`verdict` certificate machinery is transcribed from Series 07 `ws7.lean` and re-pointed to `Series08Verdict` (`spec/README.md` §3). WS7 introduces the `Circular` outcome (charter §7): the result holds only because definitions excluded the alternatives — a sharp negative about the result.*

## The object at stake

Charter §6 (WS7), protocol §0.4–§0.5, §D. Series 08's signature risks are the two the whole program watches hardest:
...
**Ambient theory.** `spec/README.md` §3 (`Series08Verdict`), all of WS1–WS6's payoffs. The verdict machinery (`Audit`, `verdict`) is Series 07's, transcribed.

## Candidates

### C1 — The mechanized audit certificate (the lead; transcribes Series 07's `Audit`)
...
- **Success condition:** `ws7_audit : Audit κ hinf` is inhabited by the WS1–WS5 payoffs. Breaking any upstream field breaks the verdict's build (the Series 07 fix for hand-set flags).
...
**Paper triage.** Transcribes Series 07's built pattern; each field is a WS1–WS5 deliverable. **Winner (the certificate).**

### C2 — The typed verdict as a function of the audit (the headline)

```lean
def verdict {hinf} (_cert : Audit κ hinf) (conservationSettled : Bool) : Series08Verdict :=
  bif conservationSettled then .perspectiveEstablished else .perspectiveEstablished  -- settled either way here
def ws7_verdict (hinf : ℵ₀ ≤ κ) : Series08Verdict := verdict (ws7_audit hinf) true
theorem ws7_verdict_eq (hinf : ℵ₀ ≤ κ) : ws7_verdict hinf = Series08Verdict.perspectiveEstablished := rfl
...
```
The honest aggregate: each Series 08 payoff, stripped of its structural word, is a bare bisimulation / order / seriality / cardinality fact — all of which *hold* (they are real facts), with the perspectival reading named as the earned surplus. This is the ledger the reviewer runs (protocol §0.3).
...
```lean
namespace Series08.WS7
-- all WS1–WS6 payoffs; the Series 07 Audit/verdict pattern — transcribed (README §6).

inductive Series08Verdict | perspectiveEstablished | monismStands | Circular deriving DecidableEq
...
def verdict {hinf : ℵ₀ ≤ κ} (_cert : Audit κ hinf) (_settled : Bool) : Series08Verdict :=
...
theorem ws7_audited_not_monism {hinf} (cert : Audit κ hinf) (b : Bool) :
    verdict cert b ≠ Series08Verdict.monismStands := by decide
theorem ws7_audited_not_circular {hinf} (cert : Audit κ hinf) (b : Bool) :
    verdict cert b ≠ Series08Verdict.Circular := by decide
...
def verdictNoCertificate : Series08Verdict := .Circular
```
...
- **Discharged:** D1 (audit), D2/D3 (verdict, `perspectiveEstablished`), D4 (two guards), D5 (strip ledger). All transcribe the Series 07 certificate pattern over WS1–WS5 payoffs.
