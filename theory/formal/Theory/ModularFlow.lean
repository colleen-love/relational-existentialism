...
/-! ## Spectral-function machinery — a function of a *diagonal* state is diagonal (handoff XXII)

Relocated here from `OneGenerator` (XXII): these are **general modular-flow structure** — that the genuine
modular operator `modPow` of a *diagonal* state is itself diagonal (`ρ^{is} = diag(dᵢ^{is})`) — depending only
on the spectral theorem, never on paper two's equilibrium assembly. They live in `ModularFlow` so the
canonical axioms can derive the modular reading of the self (`Theory.Axioms`) from the modular-flow structure
**alone**, without importing `OneGenerator`. -/
...
diagonal matrix *is* the diagonal, regardless of the eigenvector unitary's permutation freedom. -/
...
`ρ = diag(d)`: `modPow` built from `ρ`'s spectral data is literally diagonal, no eigenvector ambiguity. -/
...
/-- **The modular Hamiltonian of a diagonal state is diagonal** — `K = -log ρ = diag(-log dᵢ)`. -/
