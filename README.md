...
This builds the `P1` foundation and Program 2's `P2S0`, `P2S1`, `P2S2`, `P2S3`, `P2S4`, `P2S5` (each in its own module namespace, one module per workstream), alongside the registered Program 1 series (`Series07`, `Series12`, `Series13`) it stands on, and runs each series' `AxiomCheck`, which emits a `#print axioms` record for every headline theorem. To build a single series' axiom pass: `lake build P2S5 P2S5.AxiomCheck`. [`scripts/gate.sh`](./scripts/gate.sh) enforces the layered import chain — each series imports only its predecessor and its own roots (plus Mathlib), and nothing reaches outside the chain. The program charter, per-series charters, status ledgers, protocols, and plain-language and technical summaries live under [`program-2/`](./program-2/).
