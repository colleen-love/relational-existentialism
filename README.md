...
## Phase 3: can this physics be ours? (in progress)
...
This builds the `P1` foundation and Program 2's `P2S0`…`P2S8` (the layered chain) plus the Phase-3 probes `P2S9`, `P2S10`, `P2S11` (each in its own module namespace, one module per workstream), alongside the registered Program 1 series (`Series07`, `Series12`, `Series13`) it stands on, and runs each series' `AxiomCheck`, which emits a `#print axioms` record for every headline theorem. To build a single series' axiom pass: `lake build P2S11 P2S11.AxiomCheck`. [`scripts/gate.sh`](./scripts/gate.sh) enforces closure — each Phase-1/2 series imports only its predecessor and its own roots (plus Mathlib); the Phase-3 probes each import the finished object `P2S8` and their own roots, nothing reaching outside. The program charter, per-series charters, status ledgers, protocols, and plain-language and technical summaries live under [`program-2/`](./program-2/).
