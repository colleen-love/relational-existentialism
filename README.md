...
- **The formal spec (doctrine, signature, axioms, theorems):** [`docs/spec/`](docs/spec/) — the theorems are a categorized catalog in [`docs/spec/03-theorems.md`](docs/spec/03-theorems.md), one page per result (**3.1**–**3.7**), with rederivation citations
- **The mechanized development (Lean 4):** [`formal/`](formal/) — machine-checked and `sorry`-free, now spanning all four layers: the sparsity dichotomy (Lemmas 3.7.1 & 3.7.2), the Lawvere mirror and the cartesian firewall, a **traced symmetric monoidal category** typeclass with its **monoidal-coherence** refinement, the **free traced SMC `Cl(𝕋)`** (bare and coherent) with universal functors, a **literal matrix model** (trace = the quantum partial trace), all **five Layer-4 domain functors** (physics, chemistry, biology, AI, and the firewall), and the **spectral form of the sparsity conjecture (3.7.4) closed at the structural level** — *controlled multiplicity*: attracting selves are isolated by their own spectral contraction, hence sparse (`o(N)`), yet a two-basin witness shows they can exceed one (contingency), reconciling "sparse" with "multiple."
- **The coinductive ν-layer (Agda):** [`agda/`](agda/) — Layer 5, a second proof assistant. The doctrine's observational identity `≈` (3.2) and looped self (D1) mechanized with **native coinduction** (coinductive records + copatterns), checked `--safe --guardedness`: a constructive, final-coalgebra counterpart to Lean's lattice-theoretic `νΘ`. Now also carries the **topological sparsity** step ([spec 03.7](docs/spec/03.7-sparsity.md) step 4): under the cylinder topology on behaviours, the selves are **nowhere dense** (`selves-nowhereDense`), with the sharp dichotomy — the topological form of the sparsity conjecture that the Lean counting bound cannot reach.
