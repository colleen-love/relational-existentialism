...
- **The operator-algebraic synthesis (the relation-algebra model of self, attention, and knowing):** [`docs/relation-algebra-model.md`](docs/relation-algebra-model.md) — the one document that fixes the carrier (a relation-algebra `A`) and operators (attention `Φ_c` a unital CP map; knowing `E` a conditional expectation; the self the fixed-point subalgebra) from four commitments, tagging each result `[proved]` / `[reading]` / `[open]` and cross-linking it to the spec page and the Lean proof.
