...
This builds `Series07`, `Series09`, and `Series10` (each `ws1`–`ws7`, in its own module namespace `SeriesN.*`) and runs each series' `AxiomCheck`, which imports that series' whole build and emits a `#print axioms` record for every headline theorem. To build a single series' axiom pass: `lake build Series10 Series10.AxiomCheck` (and likewise for `Series07`, `Series09`). The completed Series 04–06 and 08 are frozen under `archive/` and no longer registered in the build. [`scripts/gate.sh`](./scripts/gate.sh) confirms no cross-series imports.
