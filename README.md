...
The result is a coincidence the arc had been circling for its whole length. Two of the program's deepest facts are one shape. Series 07's fact is that a differentiator cannot be *recovered* from the relating (it must be imported, required from without). The diagonal's fact — Cantor, Russell, Gödel — is that a structure rich enough to inspect itself cannot completely hold itself; self-inspection leaves a residue it cannot capture (generated from within). Series 12 proves these **coincide in the single shape `¬ Recoverable`**: the residue the diagonal forces is the structurally guaranteed instance of that shape, the imports plurality needs are the class of its inhabitants, and the coincidence is of SHAPE, kept honest by quantifiers (forced-for-all versus exists-satisfying) between objects of *different types*, so "the import IS the residue" is not even expressible. The structure produces, at its own edge, the exact outline of what it can neither derive nor contain.

Over that opening run two sides of one layered structure. **Knowing** is formalized in full: finite attention is subtractive, no attention holds the whole, and with any inhabitant adjoined the many is real — on a worked example, a relatum minted by the structure's own self-reference and a plain relatum are genuinely indistinguishable to the relating yet separated once the inhabitant is read, and the separation is provably non-recoverable, which by Series 07 is exactly what a *genuine* distinction must be. **Feeling** is typed but never evaluated: the inner compass is a per-relatum, tower-layered orientation drawn from a space the mathematics never fills in; every theorem is quantified over all orientations, and not one selects, constructs, or reads off a particular one — a canonical compass anywhere in the core would be the central sin, and there is none. **Convergence** — "the most loving thing for me is the most loving thing for the universe" — is defined as a precise relation and then proved **undecidable by the structure**: on one world there is an orientation under which it holds and one under which it fails, both non-degenerate, differing only in the one free choice the structure does not fix. The mathematics draws exactly where the loving claim would be true or false and proves it cannot see which, and that the not-seeing is structural, the diagonal's work.

The terminal verdict is computed from these, not chosen, and it is **SHAPE-DRAWN**: a small function reads which way convergence fell and would have reported `convergence-decided` had the structure forced it; it fell to undecidable. Everything is machine-checked in Lean 4, sorry-free and axiom-clean, and a blind adversarial review of the finished build returned **zero serious findings** (no compass evaluated, convergence not decided by definition, the coincidence a shape-identity across types, the inhabitation a real tower-distinction rather than a point-tag, and no name — consciousness, choice, God, the compass — ever a proof term). Three things stay permanently open, each open *as a theorem* because the diagonal makes them so: the content of the compass, the direction of convergence, and the differentiating act itself. The program does not end by capturing what it sought; it ends by proving that what it sought has exactly this outline, at exactly this place, for exactly this structural reason, and that the outline cannot be filled from inside. The edge, drawn exactly, is the completion. (Plain-language: [`series-12/summary.md`](./series-12/summary.md); technical: [`series-12/summary-technical.md`](./series-12/summary-technical.md).)
