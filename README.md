...
The result is an **Impossibility, and — the part that matters — it is non-circular.** The verdict is computed from an audit *certificate* whose every clause is a theorem, so if the non-circularity claims broke, the verdict would not compile. "No imported atom" is literally the program's founding principle, not an ad-hoc exclusion; and the escapes are refuted as *theorems*: a genuine labelled world is exhibited whose two atomless things stay distinct even after quotienting by behaviour, so its plurality is carried by the label — a real import, dropping *plainness* — distinct from a plain non-reduction that is killed by the quotient (dropping *behavioral identity*). **The program is explained:** weights, labels, levels each a forced drop of one ingredient; Series 6 the drop that kept all three and collapsed. **The one real escape** — limit-atomlessness, transient atoms that heal only in a limit the world never reaches — is isolated and adjudicated *import-in-time*.

What stays open, each labelled: exhaustiveness across *every possible construction* is not a formalizable claim (you cannot range over "all constructions"), so it stays a defended thesis; and the metric family for the loophole is characterized but not built. Two adversarial review passes, the second clean (a reviewer tasked solely with building a counterexample could not, and confirmed why), leave the honest verdict `payoffsEstablished`, earned. The sharpest single thing the program can now say: **a relational world can be atomless, or plural, or faithful to "an object is its relating" over plain relating — any two, never all three** — so every world that holds more than one thing has already paid for it with an atom, and the only freedom is which atom, and whether you are honest that you paid.
...
| **7** (complete) | is atomless plurality impossible without an import? | the Import Theorem | the theorem holds, non-circular — `payoffsEstablished` | [summary](./series-7/summary.md) · [technical](./series-7/summary-technical.md) · [charter](./series-7/charter.md) · [status](./series-7/charter-status.md) · [formal](./series-7/formal/) · [spec](./series-7/spec/) |
...
*Verification, per series (the AxiomCheck run emits a `#print axioms` record for every headline theorem, all on the standard three): Series 4 — 42; Series 5 — 48; Series 6 — 44; Series 7 — 37.* A publication should cite the specific commit hash and a clean-build log.
