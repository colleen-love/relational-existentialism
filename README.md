...
- **The coinductive ν-layer (Agda):** [`agda/`](agda/) — Layer 5, a second proof assistant. The doctrine's observational identity `≈` (A5) and looped self (A2) mechanized with **native coinduction** (coinductive records + copatterns), checked `--safe --guardedness`: a constructive, final-coalgebra counterpart to Lean's lattice-theoretic `νΘ`. Now also carries the **topological sparsity** step ([spec 03 §3.5](docs/spec/03-sparsity-conjecture.md) step 4): under the cylinder topology on behaviours, the selves are **nowhere dense** (`selves-nowhereDense`), with the sharp dichotomy — the topological form of the sparsity conjecture that the Lean counting bound cannot reach.
