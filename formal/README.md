...
| **Reflexive object constructed + the duality that resolves route 1** — `Unit` is a concrete reflexive object (existence not vacuous); but reflexive existence **xor** a settling-refusing observable (one contrapositive), so the diagonal *on* a reflexive object only *constructs* `Y`, never *obstructs* — the obstruction (seam) **is** the non-existence, already proved. A non-trivial reflexive object (`D∞`/`Pω`) is large, separate, and realizes `Y`, not the seam | `RelExist.ReflexiveModel.{unit_isReflexive, unit_fixpoint, reflexive_imp_settles, reflexive_xor_unsettling}` | [03.3](../docs/spec/03.3-decoherence.md) | core (no mathlib) | ✅ proved (`Quot.sound` only) |
