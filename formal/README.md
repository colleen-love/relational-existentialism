...
| **the general spectral form** (decay dynamics) | `RelExist.SpectralDecay.{spectral_pow, spectral_decay, idempotent_pow}` | `T = P + N` (peripheral projection + subdominant `‖N‖<1`) ⟹ `Tⁿ → P`: subdominant modes decay, only the peripheral eigenforms self-sustain — Conjecture 3.4's decay mechanism in any normed ring (`E`/idempotent = the `N=0` extreme) | ✅ proved |
