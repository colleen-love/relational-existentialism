...
| **`Rel` is compact closed — snake equations** | `RelExist.RelCompact.{rel_snake_right, rel_snake_left, relCompactClosed}` | the canonical model closed concretely: self-dual, diagonal cup/cap, **both zigzag / triangle identities** proved `= id` (`aesop`), plus the `Compact.CompactClosed` name bijection. The compact-closed **snake axioms** the `Int` bridge wanted (`[propext, Quot.sound]`-only); the abstract non-strict `Int(C)` composition stays open | ✅ proved |
