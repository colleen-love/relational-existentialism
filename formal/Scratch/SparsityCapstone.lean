/-
# Conjecture 3.3, closed — the cost-graded sparsity dichotomy

[03.1 §3.4](../../docs/spec/03.1-sparsity.md) states **Conjecture 3.3**: under a finite attention budget
...
dichotomy). Its proof strategy (§3.5) has four steps, all mechanized: the counting lemmas (Lean
...
owns a **private footprint** — an exclusive relating (`private_count_bound`). So Conjecture 3.3's correct
...
* `conjecture_3_3` — the **dichotomy**: density `→ 0` *with* exclusivity; unboundedly many selves within
...
With the topological nowhere-dense clause (Agda) this closes Conjecture 3.3 in its correct, cost-graded
...
`Real.stab_density_tendsto_zero` past §3.3's first gap. -/
...
/-- **Conjecture 3.3, the cost-graded dichotomy.** Over a poset of couplings with cost-sharing
...
So the counting half of Conjecture 3.3 is true exactly under a finite budget **and** an exclusive cost
